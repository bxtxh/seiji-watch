...
def upgrade():
    ...
    op.add_column(
        "bills",
        sa.Column(
            "bill_outline", sa.Text(), nullable=True, comment="議案要旨相当の長文情報"
        ),
    )
    op.add_column(
        "bills",
        sa.Column(
            "background_context", sa.Text(), nullable=True, comment="提出背景・経緯"
        ),
    )
    op.add_column(
        "bills",
        sa.Column(
            "expected_effects", sa.Text(), nullable=True, comment="期待される効果"
        ),
    )
    op.add_column(
        "bills",
    ...
    op.add_column(
        "bills",
        sa.Column(
            "submitting_party", sa.String(length=100), nullable=True, comment="提出会派"
        ),
    )
    op.add_column(
        "bills",
    ...
    op.add_column(
        "bills",
        sa.Column(
            "source_url", sa.String(length=500), nullable=True, comment="元データURL"
        ),
    )
    op.add_column(
        "bills",
        sa.Column(
            "data_quality_score", sa.Float(), nullable=True, comment="データ品質スコア"
        ),
    )
    ...
    try:
        op.add_column(
            "bills",
            sa.Column(
                "diet_session", sa.String(length=20), nullable=True, comment="国会回次"
            ),
        )
    except sa.exc.InvalidRequestError:
        # Column already exists, skip
    ...
    op.create_index(
        "idx_bills_submitting_members_gin",
        "bills",
        ["submitting_members"],
        postgresql_using="gin",
    )
    op.create_index(
        "idx_bills_committee_assignments_gin",
        "bills",
        ["committee_assignments"],
        postgresql_using="gin",
    )
