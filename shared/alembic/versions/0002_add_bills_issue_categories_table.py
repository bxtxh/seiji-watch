...
def upgrade() -> None:
    ...
    op.create_index(
        'idx_bills_categories_bill_id',
        'bills_issue_categories',
        ['bill_id'])
    op.create_index(
        'idx_bills_categories_airtable_id',
        'bills_issue_categories',
        ['issue_category_airtable_id'])
    op.create_index(
        'idx_bills_categories_confidence',
        'bills_issue_categories',
        ['confidence_score'],
        postgresql_where=sa.text('confidence_score >= 0.8'))
    op.create_index(
        'idx_bills_categories_manual',
        'bills_issue_categories',
        ['is_manual'])

    # Composite indexes for common queries
    op.create_index('idx_bills_categories_bill_confidence',
                    'bills_issue_categories', ['bill_id', 'confidence_score'])
    op.create_index('idx_bills_categories_airtable_confidence', 'bills_issue_categories', [
                    'issue_category_airtable_id', 'confidence_score'])
...
def downgrade() -> None:
    ...
    op.drop_constraint('uq_bills_issue_categories_bill_airtable',
                       'bills_issue_categories', type_='unique')
