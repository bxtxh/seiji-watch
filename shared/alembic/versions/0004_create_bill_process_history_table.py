...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('action_date', sa.DateTime(timezone=True), nullable=False, comment='実施日'),
        sa.Column('action_type', sa.String(length=50), nullable=False, comment='アクション種別'),
        sa.Column('result', sa.String(length=50), nullable=True, comment='結果'),
        sa.Column('details', postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment='詳細情報'),
