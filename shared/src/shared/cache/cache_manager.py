...
class CacheManager:
    ...
    async def get(
        ...
        if not self.redis_client:
            ...
            if loader:
                return await loader() if asyncio.iscoroutinefunction(loader) else loader()
            return None
        ...
        try:
            ...
            self.statistics.avg_response_time_ms = (
                (self.statistics.avg_response_time_ms * (self.statistics.total_requests - 1) + response_time) /
    ...
    async def set(
        ...
        try:
            ...
            entry = CacheEntry(
                ...
                compressed=self.config.use_compression and len(serialized) > self.config.compression_threshold
            )
    ...
    async def invalidate(
        ...
        try:
            ...
            logger.info(f"Invalidated {invalidated} cache entries with pattern {pattern.value}: {target}")
            return invalidated
    ...
    async def warm_cache(self, keys: Optional[List[str]] = None):
