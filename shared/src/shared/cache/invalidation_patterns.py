...
class InvalidationRule:
    ...
    condition: callable | None = None  # Condition for invalidation
...
class CacheInvalidator:
        ...
        for rule in rules:
            ...
            for pattern in rule.patterns:
                ...
                count = await self.cache_manager.invalidate(
                    InvalidationPattern.PATTERN, resolved_pattern, cascade=rule.cascade
                )
            ...
            if rule.tags:
                for tag in rule.tags:
                    count = await self.cache_manager.invalidate(
                        InvalidationPattern.TAG, tag, cascade=rule.cascade
                    )
                    total_invalidated += count

        # Log invalidation
        self._log_invalidation(
            data_type, change_type, entity_id, total_invalidated, metadata
        )
    ...
    def _log_invalidation(
        ...
        logger.info(
            f"Invalidated {count} cache entries for {key} (entity: {entity_id})"
        )

    async def invalidate_related(
        self, data_type: DataType, entity_id: str, relationship: str
    ) -> int:
        ...
        if not patterns:
            logger.warning(
                f"No relationship patterns defined for {data_type.value}.{relationship}"
            )
            return 0
        ...
        for pattern in patterns:
            ...
            count = await self.cache_manager.invalidate(
                InvalidationPattern.PATTERN, resolved, cascade=True
            )
            total_invalidated += count
    ...
    async def invalidate_by_time(
        self, older_than_minutes: int, data_types: list[DataType] | None = None
    ) -> int:
        ...
        for pattern in patterns:
            ...
            count = await self.cache_manager.invalidate(
                InvalidationPattern.PATTERN, pattern, cascade=False
            )
            total_invalidated += count
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        top_patterns = sorted(
            self.invalidation_stats.items(), key=lambda x: x[1], reverse=True
        )[:10]

        # Recent invalidation events
        recent_events = (
            self.invalidation_history[-20:] if self.invalidation_history else []
        )

        return {
            "total_rules": sum(
                len(rules) for rules in self.invalidation_rules.values()
            ),
