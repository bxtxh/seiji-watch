...
class User:
    ...
    def can_rollback(self) -> bool:
        ...
        return self.has_permission(Permission.MIGRATION_ROLLBACK) or \
               self.has_permission(Permission.ADMIN_ROLLBACK)
...
class MigrationAuth:
    ...
    def _init_default_admin(self):
        ...
        self.password_hashes = {
            admin_user.id: self._hash_password(admin_password)
        }
    ...
    def create_api_key(self, user_id: str, description: str = "") -> str:
        ...
        logger.warning(f"Store this API key securely, it won't be shown again: {api_key}")
...
class MigrationAudit:
    ...
    def log_operation(
        ...
        logger.info(
            f"AUDIT: {user.email} performed {operation} on {target} - {status}"
        )
