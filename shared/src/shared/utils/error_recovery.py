...
class RecoveryRule:
    ...
    condition: Callable[[ErrorContext], bool] | None = None
...
class ErrorRecoveryService:
    ...
    def _register_default_handlers(self):
        ...
        self.recovery_handlers[RecoveryStrategy.EXPONENTIAL_BACKOFF] = (
            self._handle_exponential_backoff
        )
        ...
        self.recovery_handlers[RecoveryStrategy.CIRCUIT_BREAK] = (
            self._handle_circuit_break
        )
    ...
    def classify_error(self, error: Exception) -> ErrorType:
        ...
        if any(
            ...
            for keyword in ["connection", "network", "socket", "refused", "reset"]
        ):
        ...
        if any(
            keyword in error_str or keyword in error_type_str
            for keyword in ["permission", "unauthorized", "forbidden", "401", "403"]
        ):
        ...
        if any(
            keyword in error_str or keyword in error_type_str
            for keyword in ["validation", "invalid", "constraint", "format"]
        ):
        ...
        if any(
            keyword in error_str or keyword in error_type_str
            for keyword in ["integrity", "consistency", "conflict", "duplicate"]
        ):
        ...
        if any(
            keyword in error_str or keyword in error_type_str
            for keyword in ["not found", "404", "missing", "unavailable"]
        ):
    ...
    async def handle_error(
        ...
        logger.error(f"Error in {component}.{operation}: {error_type.value} - {error}")
    ...
    def _find_recovery_rule(self, context: ErrorContext) -> RecoveryRule | None:
    ...
    async def _handle_retry(
        self, context: ErrorContext, rule: RecoveryRule, retry_func: Callable | None
    ) -> Any:
    ...
    async def _handle_compensate(
        ...
        if rule.compensation_handler:
            logger.info(
                f"Executing compensation for {context.component}.{context.operation}"
            )
    ...
    async def _handle_alert(
        ...
        raise Exception(f"Manual intervention required: {context.error_message}")

    async def _handle_circuit_break(
        self, context: ErrorContext, rule: RecoveryRule, retry_func: Callable | None
    ) -> Any:
        """Handle circuit breaker strategy."""
        logger.warning(f"Opening circuit breaker for {context.component}")

        self.circuit_breakers[context.component] = True

        # Schedule circuit breaker reset
        asyncio.create_task(self._reset_circuit_breaker(context.component, delay=30))

        raise Exception(f"Circuit breaker open for {context.component}")
