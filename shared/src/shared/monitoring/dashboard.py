...
from .metrics_collector import AlertSeverity, MetricsCollector
...
class MonitoringDashboard:
    ...
    def __init__(
        ...
        sync_service: AirtableSupabaseSyncService | None = None,
    ...
    async def _get_key_metrics(self) -> list[DashboardMetric]:
            ...
            if total_requests > 0:
                metrics.append(
                    DashboardMetric(
                        ...
                        unit="%",
                        trend="up"
                        if dal_metrics.get("supabase_usage_rate", 0) > 0.5
                        else "down",
                    )
                )
                ...
                metrics.append(
                    DashboardMetric(
                        ...
                        unit="%",
                        status="normal"
                        if dal_metrics.get("cache_hit_rate", 0) > 0.8
                        else "warning",
                    )
                )
        ...
        metrics.append(
            DashboardMetric(
                ...
                status="critical"
                if error_rate > 0.05
                else ("warning" if error_rate > 0.01 else "normal"),
            )
        )
        ...
        metrics.append(
            DashboardMetric(
                ...
                status="critical"
                if active_alerts > 5
                else ("warning" if active_alerts > 0 else "normal"),
            )
        )
    ...
    def get_dashboard_data(self) -> dict[str, Any]:
        ...
        return {
            ...
            "system_overview": asdict(self.system_overview)
    ...
    async def run_health_check(self) -> dict[str, Any]:
        ...
        try:
            if hasattr(self, "airtable"):
                ...
                self.metrics.update_health_status(
                    "airtable", airtable_healthy, health_results["airtable"]["message"]
                )
        except Exception as e:
            health_results["airtable"] = {"healthy": False, "message": str(e)}
        ...
        try:
            if hasattr(self, "supabase"):
                ...
                self.metrics.update_health_status(
                    "supabase", supabase_healthy, health_results["supabase"]["message"]
                )
        except Exception as e:
            health_results["supabase"] = {"healthy": False, "message": str(e)}
        ...
        if self.sync_service:
            ...
            self.metrics.update_health_status(
                "sync_service", sync_healthy, health_results["sync_service"]["message"]
            )
