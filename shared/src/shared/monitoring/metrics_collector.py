...
class MetricsCollector:
    ...
    def _init_prometheus_metrics(self):
        ...
        self.sync_lag_seconds = Gauge(
            f"{self.namespace}_sync_lag_seconds",
            "Sync lag in seconds",
            ["table"]
        )
    ...
    def record_api_request(
        ...
        # Record latency
        self.api_latency.labels(endpoint=f"{service}_{operation}", method="GET").observe(duration)
    ...
    def create_alert(
        ...
        existing = next(
            (a for a in self.alerts if a.name == name and not a.resolved),
            None
        )
    ...
    def _check_latency_alert(self, service: str, operation: str, duration: float):
        ...
        if duration > self.alert_thresholds["api_latency"]["critical"]:
            self.create_alert(
                ...
                severity=AlertSeverity.CRITICAL,
                metadata={"service": service, "operation": operation, "duration": duration}
            )
    ...
    def _calculate_average_latencies(self, metrics: List[MetricPoint]) -> Dict[str, float]:
