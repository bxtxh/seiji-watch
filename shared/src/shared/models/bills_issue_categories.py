...
class BillsPolicyCategory(BaseRecord):
    ...
    policy_category_id: str = Field(..., description="PolicyCategory identifier reference")
    
    # Airtable record IDs for actual linking
    bill_record_id: Optional[str] = Field(None, description="Airtable Bills record ID")
    policy_category_record_id: Optional[str] = Field(None, description="Airtable IssueCategories record ID")
    
    # Relationship metadata
    confidence_score: float = Field(0.8, description="Relationship confidence (0.0-1.0)")
    ...
    def __repr__(self) -> str:
        return f"<BillsPolicyCategory(bill_id='{self.bill_id}', category_id='{self.policy_category_id}', confidence={self.confidence_score})>"
