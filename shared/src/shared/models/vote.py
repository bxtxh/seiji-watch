...
class Vote(BaseRecord):
    ...
    # Vote details
    vote_result: str = Field(..., description="Vote result (yes/no/abstain/absent/present)")
    ...
    vote_stage: Optional[str] = Field(None, description="Vote stage (第一読会/第二読会/最終)")
    committee_name: Optional[str] = Field(None, description="Committee name (for committee votes)")
    ...
    is_final_vote: bool = Field(False, description="Whether this is the final vote on the bill")
    
    def __repr__(self) -> str:
        return f"<Vote(result='{self.vote_result}', bill_id={self.bill_id}, member_id={self.member_id})>"
