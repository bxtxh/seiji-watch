...
class HybridDataAccessLayer:
    ...
    async def initialize(self):
        ...
        logger.info(
            "Hybrid DAL initialized with features: "
    ...
    async def _set_in_cache(self, key: str, value: Any, ttl: int | None = None):
        """Set value in cache."""
        if not self.cache_enabled or not self.redis:
            return
        ...
        try:
            ...
            while cursor:
                cursor, keys = await self.redis.scan(
                    cursor=cursor, match=f"{self.cache_prefix}{pattern}*", count=100
                )
                if keys:
    ...
    async def get_bills(
        ...
        cache_key = self._generate_cache_key(
            "bills", {"filters": filters, "limit": limit, "offset": offset}
        )
    ...
    async def get_bill_by_id(self, bill_id: str) -> dict[str, Any] | None:
        """Get a bill by ID."""
    ...
    async def get_members(
        ...
        cache_key = self._generate_cache_key(
            "members", {"filters": filters, "limit": limit, "offset": offset}
        )
    ...
    async def get_policy_categories(
        ...
        cache_key = self._generate_cache_key(
            "policy_categories", {"layer": layer, "parent_id": parent_id}
        )
    ...
    async def _sync_to_supabase(self, table: str, airtable_record: dict[str, Any]):
        ...
        try:
            ...
            supabase_record = mapper.airtable_to_supabase(
                airtable_table, airtable_record
            )

            # Upsert to Supabase
            await self.supabase.upsert(
                table, supabase_record, on_conflict="airtable_id"
            )
    ...
    async def _sync_update_to_supabase(
        self, table: str, record_id: str, airtable_record: dict[str, Any]
    ):
        ...
        try:
            ...
            supabase_record = await self.supabase.select(
                table, filters={"airtable_id": record_id}, limit=1
            )
            ...
            if supabase_record:
                ...
                update_data = mapper.airtable_to_supabase(
                    airtable_table, {"id": record_id, "fields": airtable_record}
                )

                await self.supabase.update(table, update_data, {"id": supabase_id})
    ...
    async def _sync_update_to_airtable(
        self, table: str, supabase_id: str, supabase_data: dict[str, Any]
    ):
    ...
    def _standardize_airtable_response(
        self, records: list[dict[str, Any]], table: str
    ) -> list[dict[str, Any]]:
        """Standardize Airtable response to match Supabase format."""
        standardized = []

        for record in records:
            standardized.append(self._standardize_airtable_record(record, table))
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            **self.metrics,
            ...
            "cache_hit_rate": (
                self.metrics["cache_hits"] / total_hits if total_hits > 0 else 0
            ),
            "supabase_usage_rate": (
                self.metrics["supabase_hits"]
