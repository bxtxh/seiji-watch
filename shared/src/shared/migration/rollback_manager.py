...
class RollbackManager:
    ...
    def __init__(
        ...
        self.backup_dir = Path(backup_dir or os.getenv("ROLLBACK_BACKUP_DIR", "./backups"))
    ...
    async def _backup_configuration(self, point: RollbackPoint):
        ...
        feature_flags = {
            ...
            "FEATURE_SUPABASE_WRITE_ENABLED": os.getenv("FEATURE_SUPABASE_WRITE_ENABLED"),
            "FEATURE_AIRTABLE_FALLBACK_ENABLED": os.getenv("FEATURE_AIRTABLE_FALLBACK_ENABLED"),
    ...
    async def _backup_data(self, point: RollbackPoint):
        ...
        tables = [
            ...
            "bills_policy_categories", "meetings", "speeches",
            "votes", "issues", "issue_tags", "sync_status"
        ]
    ...
    async def execute_rollback(
        self,
        rollback_point_id: str,
        tables_to_rollback: Optional[List[str]] = None
    ) -> RecoveryOperation:
        ...
        # Find rollback point
        point = next(
            (p for p in self.rollback_points if p.id == rollback_point_id),
            None
        )
    ...
    async def _restore_schema(
        ...
        with open(schema_backup / "schema.json", "r") as f:
    ...
    def _compare_schemas(
        ...
        # Create lookup maps
        expected_map = {
            f"{r['table_name']}.{r['column_name']}": r
            for r in expected
        }
    ...
    def get_rollback_point(self, rollback_id: str) -> Optional[RollbackPoint]:
        ...
        return next(
            (p for p in self.rollback_points if p.id == rollback_id),
            None
        )
    ...
    def cleanup_old_rollbacks(self, days: int = 30):
        ...
        for point in self.rollback_points[:]:
            if point.timestamp < cutoff:
                ...
                # Remove backup files
                for path_key in ["config_backup_path", "data_backup_path", "schema_backup_path"]:
    ...
    def get_recovery_status(self, recovery_id: str) -> Optional[RecoveryOperation]:
        """Get status of a recovery operation."""
        return next(
            (op for op in self.recovery_operations if op.id == recovery_id),
            None
        )