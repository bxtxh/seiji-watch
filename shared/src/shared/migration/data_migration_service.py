...
class DataMigrationService:
    ...
    async def migrate_table(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(0, len(airtable_records), self.batch_size):
                ...
                total_batches = (len(airtable_records) + self.batch_size - 1) // self.batch_size
    ...
    async def _migrate_batch(
        ...
        for record in batch:
            try:
                ...
                if errors:
                    logger.warning(f"Validation errors for record {record.get('id')}: {errors}")
    ...
    async def validate_migration(
        ...
        for table_name in tables_to_validate:
            ...
            try:
                ...
                if airtable_count == 0:
                    ...
                    consistency = (min(supabase_count, airtable_count) / airtable_count) * 100
        ...
        valid_tables = [
            t for t in validation_results["tables"].values()
            if "consistency_score" in t
        ]
        
        if valid_tables:
            validation_results["overall_consistency"] = (
                sum(t["consistency_score"] for t in valid_tables) / len(valid_tables)
            )
