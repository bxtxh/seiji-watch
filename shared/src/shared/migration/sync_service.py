...
class AirtableSupabaseSyncService:
    ...
    def _load_config_from_env(self) -> SyncConfig:
        ...
        return SyncConfig(
            ...
            enable_bidirectional=os.getenv("SYNC_SERVICE_BIDIRECTIONAL", "false").lower() == "true"
        )
    ...
    async def _full_sync(
        ...
        supabase_records = await self.supabase.select(
            supabase_table,
            columns="id, airtable_id, updated_at"
        )
            ...
            if supabase_records:
                try:
                    ...
                    for record in response:
                        ...
                        self.id_mappings[supabase_table][record["airtable_id"]] = record["id"]
    ...
    async def resolve_conflict(
        ...
        if strategy == ConflictResolution.AIRTABLE_PRIORITY:
            ...
            await self.supabase.record_sync_conflict(
                self.mapper.get_supabase_table_name(airtable_record.get("table", "unknown")),
    ...
    async def _poll_for_changes(self):
        ...
        for table in tables:
            try:
                # Get recent changes
                last_check = self.last_sync_times.get(table, datetime.utcnow() - timedelta(minutes=1))
    ...
    async def _fetch_all_airtable_records(self, table_name: str) -> List[Dict[str, Any]]:
    ...
    async def get_sync_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_sync_times": {
                ...
                for table, time in self.last_sync_times.items()
            },
