...
const MemberPage: React.FC<MemberPageProps> = ({ 
  ...
  const [policyAnalysis, setPolicyAnalysis] = useState<PolicyAnalysis | null>(initialPolicyAnalysis);
  const [votingStats, setVotingStats] = useState<VotingStats | null>(initialVotingStats);
  const [loading, setLoading] = useState(!initialMember);
  const [error, setError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'overview' | 'policy' | 'voting' | 'activity'>('overview');
  ...
  const getStanceColor = (stance: string) => {
    switch (stance) {
      case 'strong_support': return 'bg-green-100 text-green-800 border-green-300';
      ...
      case 'oppose': return 'bg-red-50 text-red-700 border-red-200';
      case 'strong_oppose': return 'bg-red-100 text-red-800 border-red-300';
  ...
  const getStanceLabel = (stance: string) => {
    switch (stance) {
      case 'strong_support': return '強く支持';
      case 'support': return '支持';
      case 'neutral': return '中立';
      case 'oppose': return '反対';
      case 'strong_oppose': return '強く反対';
      default: return '不明';
    }
  };
      ...
      <Layout>
        <div className="max-w-4xl mx-auto py-8 text-center">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6">
            ...
            <p className="text-red-700">{error || '議員情報が見つかりません'}</p>
            <button
    ...
    <Layout>
      <div className="max-w-6xl mx-auto py-8" data-testid="member-profile">
          ...
          <div className="flex items-start space-x-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              {member.committees && member.committees.length > 0 && (
                <div className="mb-4">
                  <h3 className="text-sm font-medium text-gray-700 mb-2">所属委員会</h3>
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  <h2 className="text-lg font-semibold text-gray-900 mb-4">活動概要</h2>
                  
                  {policyAnalysis && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                      <div className="text-center">
                        <div className="text-2xl font-bold text-blue-600">
                          {Math.round(policyAnalysis.overall_activity_level * 100)}%
                        </div>
                        <div className="text-sm text-gray-600">活動度</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-green-600">
                          {Math.round(policyAnalysis.party_alignment_rate * 100)}%
                        </div>
                        <div className="text-sm text-gray-600">党方針一致率</div>
                      </div>
                      <div className="text-center">
              ...
              <div className="space-y-4">
                {votingStats && (
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <h3 className="font-semibold text-gray-900 mb-4">投票統計</h3>
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span className="text-sm text-gray-600">総投票数</span>
                        <span className="text-sm font-medium">{votingStats.total_votes}</span>
                      </div>
                      <div className="flex justify-between">
          ...
          {activeTab === 'policy' && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-semibold text-gray-900">政策立場分析</h2>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-yellow-800">政策立場分析機能について</h3>
              ...
              {policyAnalysis && policyAnalysis.strongest_positions ? (
                <div className="space-y-4">
                  <div className="text-sm text-gray-600 mb-4">
                    <span className="font-medium">※ 以下はMVP版のサンプルデータです</span>
                  </div>
                  {policyAnalysis.strongest_positions.map((position, index) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="font-medium text-gray-900">{position.issue_name}</h3>
                        <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getStanceColor(position.stance)}`}>
                          {getStanceLabel(position.stance)}
                        </span>
                      </div>
                      
                      <div className="flex items-center space-x-4 text-sm text-gray-600 mb-3">
                        <span>確信度: {Math.round(position.confidence * 100)}%</span>
                        <span>投票数: {position.vote_count}</span>
                        <span>更新: {new Date(position.last_updated).toLocaleDateString('ja-JP')}</span>
                      </div>
                      
                      {position.supporting_evidence && position.supporting_evidence.length > 0 && (
                        <div>
                          <h4 className="text-sm font-medium text-gray-700 mb-2">根拠:</h4>
                          <ul className="text-sm text-gray-600 space-y-1">
                            {position.supporting_evidence.map((evidence, evidenceIndex) => (
          ...
          {activeTab === 'voting' && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-6">投票履歴</h2>
              
              {votingStats && (
                <div className="space-y-6">
                  {/* Voting Pattern Chart */}
                  <div>
                    <h3 className="font-medium text-gray-900 mb-4">投票パターン</h3>
          ...
          {activeTab === 'activity' && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-6">活動記録</h2>
