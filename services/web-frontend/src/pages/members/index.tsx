...
const MembersPage: React.FC = () => {
  ...
  const [searchMode, setSearchMode] = useState<'auto' | 'hiragana' | 'romaji'>('auto');
  ...
  const generateMockMembers = useCallback((): Member[] => {
    const parties = ['自由民主党', '立憲民主党', '日本維新の会', '公明党', '日本共産党', '国民民主党', 'れいわ新選組', '社会民主党', '無所属'];
    const prefectures = [
    ...
    const familyNames = [
      ...
      { kanji: '高橋', kana: 'たかはし' }, { kanji: '伊藤', kana: 'いとう' }, { kanji: '渡辺', kana: 'わたなべ' },
      { kanji: '山本', kana: 'やまもと' }, { kanji: '中村', kana: 'なかむら' }, { kanji: '小林', kana: 'こばやし' },
      { kanji: '加藤', kana: 'かとう' }, { kanji: '吉田', kana: 'よしだ' }, { kanji: '山田', kana: 'やまだ' },
      { kanji: '佐々木', kana: 'ささき' }, { kanji: '山口', kana: 'やまぐち' }, { kanji: '松本', kana: 'まつもと' },
      { kanji: '井上', kana: 'いのうえ' }, { kanji: '木村', kana: 'きむら' }, { kanji: '林', kana: 'はやし' },
      { kanji: '斎藤', kana: 'さいとう' }, { kanji: '清水', kana: 'しみず' }, { kanji: '山崎', kana: 'やまざき' },
      { kanji: '森', kana: 'もり' }, { kanji: '池田', kana: 'いけだ' }, { kanji: '橋本', kana: 'はしもと' },
      { kanji: '阿部', kana: 'あべ' }, { kanji: '石川', kana: 'いしかわ' }, { kanji: '前田', kana: 'まえだ' },
      { kanji: '藤原', kana: 'ふじわら' }, { kanji: '後藤', kana: 'ごとう' }, { kanji: '近藤', kana: 'こんどう' }
    ];
    
    const givenNames = [
      { kanji: '太郎', kana: 'たろう' }, { kanji: '花子', kana: 'はなこ' }, { kanji: '一郎', kana: 'いちろう' },
      { kanji: '美咲', kana: 'みさき' }, { kanji: '健', kana: 'けん' }, { kanji: '愛', kana: 'あい' },
      { kanji: '誠', kana: 'まこと' }, { kanji: '恵', kana: 'めぐみ' }, { kanji: '学', kana: 'まなぶ' },
      { kanji: '智子', kana: 'ともこ' }, { kanji: '正雄', kana: 'まさお' }, { kanji: '由美', kana: 'ゆみ' },
      { kanji: '博', kana: 'ひろし' }, { kanji: '理恵', kana: 'りえ' }, { kanji: '和也', kana: 'かずや' },
      { kanji: '純子', kana: 'じゅんこ' }, { kanji: '大輔', kana: 'だいすけ' }, { kanji: '美穂', kana: 'みほ' },
      { kanji: '哲也', kana: 'てつや' }, { kanji: '真由美', kana: 'まゆみ' }
    ];

    const members: Member[] = [];
    
    for (let i = 1; i <= 723; i++) { // Realistic number of Diet members
      ...
      const house = i <= 465 ? 'house_of_representatives' : 'house_of_councillors'; // 465 HR, 248 HC
      
      const constituencyType = house === 'house_of_representatives' 
        ? Math.random() > 0.8 ? '比例代表' : `第${Math.floor(Math.random() * 15) + 1}区`
        : Math.random() > 0.5 ? '選挙区' : '比例代表';
  ...
  const getPartyColor = (party: string) => {
    switch (party) {
      case '自由民主党': return 'bg-red-100 text-red-800 border-red-200';
      case '立憲民主党': return 'bg-blue-100 text-blue-800 border-blue-200';
      case '日本維新の会': return 'bg-orange-100 text-orange-800 border-orange-200';
      case '公明党': return 'bg-yellow-100 text-yellow-800 border-yellow-200';
      case '日本共産党': return 'bg-red-100 text-red-800 border-red-200';
      case '国民民主党': return 'bg-green-100 text-green-800 border-green-200';
      case 'れいわ新選組': return 'bg-purple-100 text-purple-800 border-purple-200';
      case '社会民主党': return 'bg-pink-100 text-pink-800 border-pink-200';
  ...
  return (
    <Layout>
      <div className="max-w-6xl mx-auto py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">国会議員一覧</h1>
          <p className="text-gray-600">
            {allMembers.length}名の議員が登録されています
            {filteredMembers.length !== allMembers.length && ` (${filteredMembers.length}名が条件に一致)`}
          </p>
        </div>
              ...
              <div>
                ...
                >
                  ...
                  <option value="house_of_representatives">
                    衆議院 ({allMembers.filter(m => m.house === 'house_of_representatives').length}名)
                  </option>
                  <option value="house_of_councillors">
                    参議院 ({allMembers.filter(m => m.house === 'house_of_councillors').length}名)
                  </option>
                </select>
              </div>
                ...
                >
                  ...
                  {uniqueParties.map(party => (
                    <option key={party} value={party}>
                      {party} ({allMembers.filter(m => m.party === party).length}名)
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="text-sm text-gray-600 bg-blue-50 p-3 rounded">
                <div className="flex items-center space-x-4">
                  ...
                  {wanakana.isRomaji(searchQuery) && (
                    <span>📝 ローマ字変換: {searchQuery} → {wanakana.toHiragana(searchQuery)}</span>
                  )}
                </div>
              </div>
            )}
        ...
        {error ? (
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden" data-testid="member-list">
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                                ...
                                <div className="flex flex-wrap items-center gap-3 mt-2">
                                  ...
                                  <span className={`px-2 py-1 text-xs rounded-full border ${getPartyColor(member.party)}`}>
                                    {member.party}
                                  </span>
            ...
            {filteredMembers.length > 0 && (
              <div className="bg-gray-50 px-6 py-3 border-t border-gray-200 text-sm text-gray-600 text-center">
                📜 仮想スクロール: {filteredMembers.length}名中 {virtualizer.getVirtualItems().length}名を表示中
