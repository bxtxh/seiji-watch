...
const IssueDetailPage = ({ issue: initialIssue, relatedBills: initialRelatedBills, issueTags: initialIssueTags, error }: IssueDetailProps) => {
  const router = useRouter()
  const { id } = router.query
  const [issue, setIssue] = useState<ExtendedIssue | null>(initialIssue)
  const [relatedBills, setRelatedBills] = useState<Bill[]>(initialRelatedBills || [])
  const [issueTags, setIssueTags] = useState<IssueTag[]>(initialIssueTags || [])
  const [loading, setLoading] = useState(!initialIssue && !error)
  const [activeTab, setActiveTab] = useState<'overview' | 'bills' | 'discussions'>('overview')
  ...
  const mockDiscussions = useMemo(() => [
    {
      ...
      summary: 'カーボンニュートラル目標達成のための具体的政策について活発な議論が行われました。',
      committee: '環境委員会',
    ...
    },
    {
      ...
      title: '本会議での環境関連法案審議',
      speakers: ['鈴木一郎', '高橋美子'],
      summary: '温室効果ガス削減に向けた新たな法的枠組みについて議論されました。',
      committee: '本会議',
      ...
      <Layout title="イシュー詳細 - エラー">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center py-12">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">イシューが見つかりません</h1>
            <p className="text-gray-600 mb-6">
              {error || '指定されたイシューが存在しないか、削除された可能性があります。'}
            </p>
            <button
              onClick={() => router.back()}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200">
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8 px-6" role="tablist" aria-label="イシュー詳細タブ">
              {[
                { id: 'overview', label: '概要', icon: InformationCircleIcon },
                { id: 'bills', label: `関連法案 (${relatedBills.length})`, icon: DocumentTextIcon },
                { id: 'discussions', label: '議論履歴', icon: ChatBubbleLeftRightIcon }
              ].map((tab) => (
                <button
                  key={tab.id}
                  role="tab"
          ...
          <div className="p-6">
            ...
            {activeTab === 'discussions' && (
              <div role="tabpanel" id="discussions-panel" aria-labelledby="discussions-tab">
                <DiscussionTimeline discussions={mockDiscussions} loading={discussionsLoading} />
              </div>
            )}
          </div>
        </div>
