...
const CategoryDetailPage = () => {
  ...
  const [parentCategory, setParentCategory] = useState<IssueCategory | null>(null);
  ...
  const fetchCategoryData = async (categoryId: string) => {
    try {
      ...
      const categoryResponse = await fetch(`${apiBaseUrl}/api/issues/categories/${categoryId}`);
      ...
      if (categoryData.fields.Parent_Category && categoryData.fields.Parent_Category.length > 0) {
        const parentId = categoryData.fields.Parent_Category[0];
        try {
          const parentResponse = await fetch(`${apiBaseUrl}/api/issues/categories/${parentId}`);
      ...
      try {
        const childrenResponse = await fetch(`${apiBaseUrl}/api/issues/categories/${categoryId}/children`);
      ...
      try {
        const billsResponse = await fetch(`${apiBaseUrl}/api/bills?max_records=10`);
        if (billsResponse.ok) {
          const billsData = await billsResponse.json();
          // Filter bills by category (simplified logic)
          const filtered = billsData.filter((bill: BillRecord) => 
            bill.fields.Category && bill.fields.Category.includes(categoryData.fields.Title_JA)
          );
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50">
          <div className="container mx-auto px-4 py-8">
            <div className="bg-red-50 border border-red-200 rounded-lg p-6">
              ...
              <p className="text-red-700">{error || 'カテゴリが見つかりません'}</p>
              <Link href="/issues/categories" className="mt-4 inline-block bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors">
                カテゴリ一覧に戻る
              </Link>
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 py-8">
              ...
              {breadcrumbs.map((breadcrumb, index) => (
                <li key={index} className="inline-flex items-center">
                  ...
                  {index === breadcrumbs.length - 1 ? (
                    <span className="text-gray-500 text-sm font-medium">{breadcrumb.label}</span>
                  ) : (
                    <Link href={breadcrumb.href} className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                      {breadcrumb.label}
                    </Link>
                  )}
                </li>
                ...
                <div>
                  <div className="flex items-center mb-2">
                    ...
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                      ...
                      {category.fields.Layer === 'L1' ? '主要分野' : 'サブトピック'}
                    </span>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              {childCategories.length > 0 && (
                <div className="bg-white rounded-xl shadow-md p-6">
                  <h2 className="text-2xl font-bold text-gray-800 mb-6">サブトピック</h2>
                ...
                {relatedBills.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {relatedBills.map((billRecord) => (
                      <BillCard key={billRecord.id} bill={convertBillRecordToBill(billRecord)} />
                    ))}
                  </div>
                ) : (
            ...
            <div className="space-y-6">
              {/* Quick Stats */}
              <div className="bg-white rounded-xl shadow-md p-6">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">統計情報</h3>
              ...
              {/* Navigation */}
              <div className="bg-white rounded-xl shadow-md p-6">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">ナビゲーション</h3>
