...
const DiscussionTimeline: React.FC<DiscussionTimelineProps> = ({ discussions, loading = false }) => {
  ...
  if (loading) {
    return (
      <div className="space-y-6" aria-live="polite" aria-label="議論履歴を読み込み中">
        {[1, 2, 3].map((i) => (
  ...
  if (discussions.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="mt-4 text-lg font-medium text-gray-900">議論の記録が見つかりません</h3>
  ...
  return (
    <section className="space-y-6" aria-labelledby="discussions-heading">
      <header className="flex items-center justify-between">
        <h2 id="discussions-heading" className="text-lg font-semibold text-gray-900 flex items-center">
          <ChatBubbleLeftRightIcon className="w-5 h-5 mr-2" aria-hidden="true" />
          国会での議論
          <span className="ml-2 bg-gray-200 text-gray-700 px-2 py-1 rounded-full text-sm" aria-label={`${discussions.length}件の議論`}>
            {discussions.length}
          </span>
        </h2>
      </header>
      ...
      <div className="relative" role="list" aria-label="議論履歴">
        ...
        <div className="space-y-8">
          {discussions.map((discussion, index) => (
            <article key={discussion.id} className="relative flex items-start space-x-4" role="listitem">
              {/* Timeline dot */}
              <div 
              ...
              <div className="flex-1 min-w-0 bg-white border border-gray-200 rounded-lg p-4 shadow-sm"
                ...
                <header className="flex justify-between items-start mb-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
                      <time dateTime={discussion.date}>{formatDate(discussion.date)}</time>
                      <span 
                      ...
                      {discussion.committee && (
                        <span className="text-gray-500" aria-label={`委員会: ${discussion.committee}`}>
                          {discussion.committee}
                        </span>
                      )}
                    </div>
                    <h3 id={`discussion-title-${discussion.id}`} className="text-base font-semibold text-gray-900">
                      {discussion.title}
                    </h3>
                  </div>
                </header>
                ...
                {discussion.speakers.length > 0 && (
                  <div className="mb-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h4 className="text-sm font-medium text-gray-700">発言者</h4>
                    </div>
                    <div className="flex flex-wrap gap-2" role="list" aria-label="発言者一覧">
                      {discussion.speakers.slice(0, 5).map((speaker, speakerIndex) => (
                        <span
                          key={speakerIndex}
                          role="listitem"
                ...
                <footer className="flex justify-between items-center pt-3 border-t border-gray-100">
                  <div className="text-xs text-gray-500">
                    {index === 0 && <span className="font-medium">最新の議論</span>}
                  </div>
