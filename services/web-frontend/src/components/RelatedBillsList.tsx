...
const RelatedBillsList: React.FC<RelatedBillsListProps> = ({ bills, loading = false }) => {
  ...
  if (loading) {
    return (
      <div className="space-y-4" aria-live="polite" aria-label="関連法案を読み込み中">
        {[1, 2, 3].map((i) => (
          <div key={i} className="bg-white border border-gray-200 rounded-lg p-4 animate-pulse" aria-hidden="true">
  ...
  if (bills.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="mt-4 text-lg font-medium text-gray-900">関連法案が見つかりません</h3>
  ...
  return (
    <section className="space-y-4" aria-labelledby="related-bills-heading">
      <header className="flex items-center justify-between">
        <h2 id="related-bills-heading" className="text-lg font-semibold text-gray-900 flex items-center">
          <DocumentTextIcon className="w-5 h-5 mr-2" aria-hidden="true" />
          関連法案
          <span className="ml-2 bg-gray-200 text-gray-700 px-2 py-1 rounded-full text-sm" aria-label={`${bills.length}件の関連法案`}>
            {bills.length}
          </span>
        </h2>
      </header>
          ...
          >
            <div className="flex justify-between items-start mb-3">
              <div className="flex-1 min-w-0">
                <h3 id={`bill-title-${bill.id}`} className="text-base font-semibold text-gray-900 line-clamp-2">
                  {bill.title}
                </h3>
              ...
              <div className="flex flex-col gap-2 ml-4" role="group" aria-label="法案ステータス">
                <span 
                ...
                >
                  {formatCategory(bill.policy_category?.title_ja || bill.category || 'その他')}
                </span>
              </div>
            </div>
