...
import { CalendarIcon, TagIcon, ClockIcon, ExclamationTriangleIcon } from '@heroicons/react/24/outline'
...
const IssueDetailCard: React.FC<IssueDetailCardProps> = ({ issue, issueTags }) => {
  ...
  return (
    ...
    >
      ...
      {/* Description */}
      {issue.description && (
        <section className="prose prose-sm max-w-none" aria-labelledby="description-heading">
          <h2 id="description-heading" className="text-lg font-semibold text-gray-900 mb-2">概要</h2>
      ...
      {/* Schedule */}
      {issue.schedule && (
        <section className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg" aria-labelledby="schedule-heading">
          <CalendarIcon className="w-5 h-5 text-gray-600 flex-shrink-0" aria-hidden="true" />
          <div>
            <h3 id="schedule-heading" className="text-sm font-medium text-gray-900">審議スケジュール</h3>
            <time className="text-sm text-gray-600" dateTime={issue.schedule.from}>
      ...
      {tags.length > 0 && (
        <section aria-labelledby="tags-heading">
          <h2 id="tags-heading" className="text-lg font-semibold text-gray-900 mb-3 flex items-center">
            <TagIcon className="w-5 h-5 mr-2" aria-hidden="true" />
            イシュータグ
          </h2>
          <div className="flex flex-wrap gap-2" role="list" aria-label="イシュータグ一覧">
            {tags.map((tag) => (
              <span
                key={tag.id}
                role="listitem"
      ...
      <footer className="pt-4 border-t border-gray-200">
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          ...
          {issue.extraction_confidence && (
            <div>
              ...
              <dd className="ml-2 text-gray-600" aria-label={`信頼度 ${Math.round(issue.extraction_confidence * 100)}パーセント`}>
        ...
        {issue.review_notes && (
          <div className="mt-4 p-3 bg-blue-50 rounded-lg" role="note" aria-labelledby="review-notes-heading">
            <h3 id="review-notes-heading" className="text-sm font-medium text-blue-900 mb-1">レビューノート</h3>
