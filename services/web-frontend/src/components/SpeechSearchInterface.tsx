...
export default function SpeechSearchInterface() {
  ...
  const [searchStats, setSearchStats] = useState<SpeechSearchStats | null>(null);
  ...
  const handleSearch = async (searchQuery: string, topics: string[] = []) => {
    ...
    // Rate limiting check
    const clientId = 'speech_search_' + (typeof window !== 'undefined' ? window.location.hostname : 'server');
      ...
      if (response.success) {
        ...
        if (topics.length > 1) {
          filteredSpeeches = speeches.filter(speech => 
            speech.topics && topics.every((topic: string) => 
              speech.topics!.some((speechTopic: string) => speechTopic.includes(topic))
            )
          );
        }
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="card p-4 sm:p-6">
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
          <div>
            <label htmlFor="speech-search" className="block text-sm sm:text-base font-medium text-gray-700 mb-2">
              発言を検索
            </label>
            ...
            {validationError ? (
              ...
              <p id="speech-search-help" className="mt-2 text-xs sm:text-sm text-gray-600">
                国会での発言内容を検索できます（{INPUT_LIMITS.SEARCH_QUERY}文字まで）
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-2 sm:space-y-0">
            ...
            >
              <svg className={`h-3 w-3 sm:h-4 sm:w-4 transition-transform ${showTopicFilter ? 'rotate-90' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {showTopicFilter && (
            <div className="border-t pt-3 sm:pt-4">
              <p className="text-xs sm:text-sm font-medium text-gray-700 mb-3">トピックを選択</p>
          ...
          <button
            type="submit"
            disabled={loading || (!query.trim() && selectedTopics.length === 0) || !!validationError}
      ...
      {results.length > 0 && (
        <div className="space-y-3 sm:space-y-4">
          <h2 className="text-base sm:text-lg font-semibold text-gray-900">発言検索結果</h2>
      ...
      {/* Empty State */}
      {(query || selectedTopics.length > 0) && !loading && results.length === 0 && !error && (
        <div className="text-center py-8 sm:py-12">
          <svg className="mx-auto h-10 w-10 sm:h-12 sm:w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <h3 className="mt-2 text-sm sm:text-base font-medium text-gray-900">該当する発言が見つかりませんでした</h3>
