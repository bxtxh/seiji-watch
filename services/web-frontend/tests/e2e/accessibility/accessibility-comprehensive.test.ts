...
test.describe('Accessibility Tests (WCAG 2.1 AA)', () => {
  ...
  test('should pass automated WCAG 2.1 AA checks on homepage', async ({ page }) => {
  ...
  test('should support keyboard navigation', async ({ page }) => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (focusedCount > 0) {
        ...
        interactiveElements.push(`${tagName}${role ? `[role="${role}"]` : ''}${ariaLabel ? `[aria-label="${ariaLabel}"]` : ''}`);
        ...
        if (tagName === 'button' || role === 'button') {
          ...
          console.log(`Found focusable button: ${tagName}${role ? `[role="${role}"]` : ''}`);
        }
  ...
  test('should support screen reader navigation', async ({ page }) => {
    ...
    const skipLink = page.locator('a:has-text("メインコンテンツへスキップ"), a:has-text("スキップ")');
    ...
    for (let i = 1; i < headingLevels.length; i++) {
      ...
      if (currentLevel > previousLevel + 1) {
        console.warn(`Heading level skipped: h${previousLevel} followed by h${currentLevel}`);
      }
    }
    
    console.log('Heading structure:', headingLevels.map(level => `h${level}`).join(' → '));
  });
  ...
  test('should have sufficient color contrast', async ({ page }) => {
    ...
    const primaryColors = [
      ...
      { selector: '.bg-primary-green', description: 'Primary green background' },
      ...
      { selector: '.bg-primary-yellow', description: 'Primary yellow background' }
    ];
  ...
  test('should handle focus management properly', async ({ page }) => {
    ...
    const modalTriggers = page.locator('button:has-text("詳細"), button:has-text("詳細を見る")');
    ...
    if (modalCount > 0) {
      ...
      if (modalOpenCount > 0) {
        ...
        const focusWithinModal = await modalContainer.locator(':focus').count() > 0;
  ...
  test('should provide error messages and form validation feedback', async ({ page }) => {
    ...
    const errorMessages = page.locator('[role="alert"], .text-red-600, .error-message');
  ...
  test('should support Japanese language accessibility features', async ({ page }) => {
    // Check for lang attribute
    const htmlLang = await page.getAttribute('html', 'lang');
    expect(htmlLang).toBe('ja');
    
    // Check for furigana support (if implemented)
    const furiganaToggle = page.locator('button:has-text("ふりがな"), [aria-label*="ふりがな"]');
