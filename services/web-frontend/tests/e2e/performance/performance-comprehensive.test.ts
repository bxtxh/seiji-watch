...
test.describe('Performance Tests', () => {
  ...
  test('should maintain performance on mobile devices', async ({ page }) => {
    ...
    expect(domContentLoadedTime).toBeLessThan(performanceThresholds.mobile.domContentLoaded);
  ...
  test('should optimize bundle size and loading', async ({ page, context }) => {
    ...
    const jsFiles = networkRequests.filter(req => 
      req.type.includes('javascript') || req.url.endsWith('.js')
    );
    
    // Check CSS bundle size
    const cssFiles = networkRequests.filter(req => 
      req.type.includes('css') || req.url.endsWith('.css')
    );
  ...
  test('should efficiently handle responsive design changes', async ({ page }) => {
    await page.goto('/');
    await helpers.waitForAppReady();
