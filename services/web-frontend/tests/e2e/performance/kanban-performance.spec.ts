...
test.describe('Kanban Board Performance Tests', () => {
  ...
  test('Cumulative Layout Shift (CLS) - no layout shifts during load', async ({ page }) => {
    await page.goto('/');
    
    // Get initial viewport measurements
    const initialMetrics = await page.evaluate(() => {
      const kanban = document.querySelector('[aria-label="国会イシュー Kanban ボード"]');
  ...
  test('API response caching effectiveness', async ({ page }) => {
    ...
    console.log(`API requests - First load: ${firstLoadRequestCount}, Additional: ${additionalRequests}`);
  });
  ...
  test('Scroll performance - 60fps target', async ({ page }) => {
    ...
    const performanceData = await page.evaluate(() => {
      ...
      function measureFrame() {
        ...
        if (frames.length < 60) { // Monitor for 60 frames
    ...
    console.log(`Average FPS: ${result.fps.toFixed(2)}, Frame time: ${result.avgFrameTime.toFixed(2)}ms`);
  });
  ...
  test('Resource loading optimization', async ({ page }) => {
    ...
    const cssResources = resourceSizes.filter(r => 
      r.contentType?.includes('css') || r.url.includes('.css')
    );
