...
test.describe('Performance Tests', () => {
  ...
  test('should meet page load performance targets', async ({ page }) => {
    ...
    expect(domContentLoadedTime).toBeLessThan(performanceThresholds.domContentLoaded);
    ...
    const additionalMetrics = await page.evaluate(() => {
      return new Promise((resolve) => {
        if ('PerformanceObserver' in window) {
          ...
          // Fallback timeout
          setTimeout(() => resolve({ largestContentfulPaint: 0, cumulativeLayoutShift: 0 }), 5000);
        } else {
  ...
  test('should perform well on mobile devices', async ({ page }) => {
    ...
    expect(mobileLoadTime).toBeLessThan(performanceThresholds.mobile.domContentLoaded);
  ...
  test('should optimize images', async ({ page }) => {
    ...
    for (const img of imageRequests) {
      console.log(`Image: ${img.url}, Type: ${img.contentType}, Size: ${img.size}`);
      
      // Should use modern formats when possible
      const isModernFormat = img.contentType.includes('webp') || 
  ...
  test('should optimize for search performance', async ({ page }) => {
    ...
    await page.waitForSelector('text=検索結果が見つかりませんでした', { timeout: 5000 });
  ...
  test('should optimize for different device capabilities', async ({ page }) => {
    const deviceTests = [
      { name: 'High-end mobile', viewport: testViewports.mobile, network: 'fast3g' },
      { name: 'Low-end mobile', viewport: { width: 320, height: 568 }, network: 'slow3g' },
