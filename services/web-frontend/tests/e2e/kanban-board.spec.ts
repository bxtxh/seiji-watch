...
test.describe('Kanban Board - EPIC 8', () => {
  ...
  test('Kanban board loads and displays correctly', async ({ page }) => {
    ...
    await expect(page.locator('[aria-label="国会イシュー Kanban ボード"]')).toBeVisible();
  ...
  test('Issue cards are clickable and have proper accessibility', async ({ page }) => {
    // Wait for data to load
    await page.waitForTimeout(3000);
  ...
  test('Kanban board is horizontally scrollable', async ({ page }) => {
    ...
    const scrollContainer = page.locator('[role="list"][aria-label="ステージ別イシュー一覧"]');
  ...
  test('Card hover triggers prefetch', async ({ page }) => {
    ...
    page.on('request', request => {
      if (request.url().includes('/issues/') && request.resourceType() === 'document') {
  ...
  test('More items indicator works when stage has many issues', async ({ page }) => {
    // Wait for data to load
    await page.waitForTimeout(3000);
  ...
  test('Empty stage shows proper empty state', async ({ page }) => {
    ...
    if (await emptyState.isVisible()) {
      ...
      await expect(page.locator('text=新しいイシューが追加されると')).toBeVisible();
    }
  });
  ...
  test('Kanban board metadata displays correctly', async ({ page }) => {
    ...
    if (await metadata.isVisible()) {
      ...
      expect(await metadata.textContent()).toMatch(/最終更新: \d{4}\/\d{1,2}\/\d{1,2}/);
  ...
  test('Kanban board responsive design on mobile', async ({ page }) => {
    ...
    const kanbanContainer = page.locator('[role="list"][aria-label="ステージ別イシュー一覧"]');
  ...
  test('Kanban board performance - load time under 3 seconds', async ({ page }) => {
