...
test.describe('API Integration Tests', () => {
  ...
  test('should handle API health check', async ({ page }) => {
    ...
    await expect(page.locator('text=システムは正常に動作しています')).toBeVisible();
  });
  ...
  test('should handle API errors gracefully', async ({ page }) => {
    ...
    await expect(page.locator('text=システムへの接続に失敗しました')).toBeVisible();
  });
  ...
  test('should handle rate limiting', async ({ page }) => {
    ...
    await page.route('**/api/search', async route => {
      ...
      if (requestCount > 3) {
        await route.fulfill({
          ...
          body: JSON.stringify({
            success: false,
            message: 'API呼び出し回数の上限に達しました。しばらくお待ちください。'
          })
        });
      } else {
        await route.fulfill({
    ...
    await expect(page.locator('text=上限に達しました')).toBeVisible({ timeout: 5000 });
  });
