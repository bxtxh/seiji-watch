...
test.describe('Security Tests', () => {
  ...
  test('should prevent XSS attacks', async ({ page }) => {
    ...
    for (const payload of xssPayloads) {
      ...
      const hasValidationError = await page.locator(TestHelpers.selectors.validationError).count() > 0;
  ...
  test('should validate and sanitize input', async ({ page }) => {
    ...
    for (const input of invalidInputs) {
      ...
      if (input === '') {
        ...
        const hasError = await page.locator(TestHelpers.selectors.validationError).count() > 0;
        const hasGeneralError = await page.locator(TestHelpers.selectors.errorMessage).count() > 0;
  ...
  test('should enforce rate limiting', async ({ page }) => {
    ...
    const rateLimitMessage = await page.locator('text=/上限|制限|limit/i').count();
  ...
  test('should handle CSRF protection', async ({ page }) => {
    ...
    for (let i = 0; i < formCount; i++) {
      ...
      const hasCSRFField = await form.locator('input[name="_csrf"]').count() > 0;
  ...
  test('should protect against information disclosure', async ({ page }) => {
    ...
    const debugElements = await page.locator('[data-debug], .debug, #debug').count();
    const devOnlyElements = await page.locator('text=/debug|dev|test/i').count();
  ...
  test('should handle error messages securely', async ({ page }) => {
    ...
    await page.route('/api/search', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          success: false,
          message: 'Database connection failed at server.example.com:5432 with credentials user:admin',
    ...
    await expect(page.locator(TestHelpers.selectors.errorMessage)).toBeVisible();
    
    const errorText = await page.locator(TestHelpers.selectors.errorMessage).textContent();
