...
test.describe("Security Tests", () => {
  ...
  test("should prevent XSS attacks in search input", async ({ page }) => {
    ...
    for (const maliciousInput of maliciousInputs) {
      ...
      const hasSecurityError =
        (await page.locator("text=セキュリティ上の理由").count()) > 0;
  ...
  test("should prevent SQL injection attempts", async ({ page }) => {
    ...
    for (const sqlInput of sqlInjectionInputs) {
      ...
      // Should show security error or validation error
      const hasSecurityError =
        (await page.locator("text=不正な入力が検出されました").count()) > 0;
      const hasValidationError =
        (await page.locator(".text-red-600").count()) > 0;
  ...
  test("should implement rate limiting", async ({ page }) => {
    ...
    const hasRateLimit =
      (await page.locator("text=上限に達しました").count()) > 0;
    const hasRateLimitApi =
      (await page.locator("text=API呼び出し回数の上限").count()) > 0;
  ...
  test("should prevent clickjacking attacks", async ({ page }) => {
    ...
    const iframeLoaded = await iframe
      .evaluate((iframe) => {
        try {
          ...
          results: [
            {
              ...
              description:
                'Description with <img src="x" onerror="alert(\'xss2\')"> dangerous content',
              certainty: 0.9,
