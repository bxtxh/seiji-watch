...
export class TestHelpers {
  ...
  async performSearch(query: string): Promise<void> {
    ...
    await Promise.race([
      this.page.waitForSelector(TestHelpers.selectors.searchResults, { timeout: 10000 }),
      this.page.waitForSelector('text=検索結果が見つかりませんでした', { timeout: 10000 }),
      this.page.waitForSelector(TestHelpers.selectors.errorMessage, { timeout: 10000 })
    ]);
  }
  ...
  async expectElementToBeAccessible(selector: string): Promise<void> {
    ...
    if (tagName === 'input') {
      ...
      const associatedLabel = await this.page.locator(`label[for="${await element.getAttribute('id')}"]`).count();
   ...
   */
  async simulateNetworkCondition(condition: 'offline' | 'slow3g' | 'fast3g'): Promise<void> {
...
export class DietTrackerAssertions {
  static async expectSearchResults(page: Page, query: string): Promise<void> {
    ...
    const hasResults = await page.locator(TestHelpers.selectors.searchResults).count() > 0;
    const hasEmptyState = await page.locator(TestHelpers.selectors.emptyState).count() > 0;
    const hasError = await page.locator(TestHelpers.selectors.errorMessage).count() > 0;
    
    expect(hasResults || hasEmptyState || hasError).toBe(true);
  }

  static async expectValidJapaneseContent(page: Page, selector: string): Promise<void> {
