...
class EnvironmentValidator:
    ...
    def _validate_formats(cls) -> list[str]:
        ...
        if log_level not in ["DEBUG", "INFO", "WARNING", "ERROR"]:
            errors.append(
                f"Invalid LOG_LEVEL: {log_level}. Must be one of: DEBUG, INFO, WARNING, ERROR"
            )
        ...
        if environment not in [
            "development",
            "staging",
            "staging-external-test",
            "production",
        ]:
            errors.append(
                f"Invalid ENVIRONMENT: {environment}. Must be one of: development, staging, staging-external-test, production"
            )
    ...
    def print_config(cls, mask_secrets: bool = True):
        ...
        for key, value in config.items():
            if mask_secrets and any(
                secret in key for secret in ["pat", "secret", "token", "key"]
            ):
                if value:
