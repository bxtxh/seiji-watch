...
class AirtableMigrator:
    ...
    def update_record(
        self, record_id: str, fields_to_update: dict, dry_run: bool = True
    ) -> bool:
        """レコードを更新（ドライラン対応）"""
        if dry_run:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Notes フィールドから構造化フィールドへのマイグレーション"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    parser.add_argument(
        "--pattern",
        choices=["structured_emoji", "pipe_separated", "detailed_newline", "all"],
        default="all",
        help="処理するパターンを指定",
    )
