...
async def verify_member_names():
    ...
    async with aiohttp.ClientSession() as session:
            ...
            if name:
                if re.search(r'\d+$', name):
                    names_with_numbers.append({
                        'id': record['id'],
                        'name': name,
                        'constituency': record.get('fields', {}).get('Constituency', ''),
        ...
        if clean_percentage == 100:
            print(f"🏆 PERFECT! All member names are properly formatted.")
        elif clean_percentage >= 98:
            print(f"🎯 EXCELLENT! Almost all names are clean.")
        elif clean_percentage >= 95:
            print(f"👍 GOOD! Most names are clean.")
        else:
