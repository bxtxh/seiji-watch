...
class FullTextSearchEngine:
        ...
        if query.filters:
            for key, value in query.filters.items():
                if key == 'category' and value:
                    filter_conditions.append(f"category = :filter_category")
                    params['filter_category'] = value
                elif key == 'status' and value:
                    filter_conditions.append(f"status = :filter_status")
                    params['filter_status'] = value
                elif key == 'submitter' and value:
                    filter_conditions.append(f"submitter = :filter_submitter")
                    params['filter_submitter'] = value
                elif key == 'diet_session' and value:
                    filter_conditions.append(f"diet_session = :filter_session")
                    params['filter_session'] = value
                elif key == 'house_of_origin' and value:
                    filter_conditions.append(f"house_of_origin = :filter_house")
                    params['filter_house'] = value
        
        # Add date range filter
        if query.date_range:
            start_date, end_date = query.date_range
            filter_conditions.append(f"submitted_date BETWEEN :start_date AND :end_date")
    ...
    async def reindex_bills(self, session: Session, bill_ids: Optional[List[str]] = None):
