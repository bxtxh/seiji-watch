...
from ..services.issue_versioning_service import IssueVersioningService, VersionChangeType
...
class BatchJobConfig:
    ...
    def __post_init__(self):
        if not self.job_id:
            self.job_id = f"{self.job_type.value}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
class IssueRelationshipBatchProcessor:
    ...
    def __init__(self, airtable_client: Optional[AirtableClient] = None,
                 issue_manager: Optional[AirtableIssueManager] = None,
                 versioning_service: Optional[IssueVersioningService] = None,
                 discord_bot: Optional[DiscordNotificationBot] = None):
    ...
    async def _execute_relationship_update(self, execution: BatchJobExecution):
        ...
        issues = await self.issue_manager.list_issues_by_status("approved", max_records=1000)
    ...
    async def _execute_quality_assessment(self, execution: BatchJobExecution):
        ...
        issues = await self.issue_manager.list_issues_by_status("approved", max_records=500)
    ...
    async def _cleanup_job_history(self):
        ...
        self.job_history = [
            execution for execution in self.job_history
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            return airtable_healthy and issue_manager_healthy and versioning_healthy and stuck_jobs == 0
