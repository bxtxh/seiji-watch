...
from ...shared.src.shared.models.bill_process_history import BillProcessHistory, HistoryEventType, HistoryChangeType
...
class BillHistoryRecorder:
    ...
    def __init__(self, database_url: str):
        ...
        self.config = {
            ...
            'significant_fields': [
                'status', 'stage', 'final_vote_date', 'implementation_date',
                'committee_assignments', 'vote_results', 'promulgated_date'
            ],
            'tracked_fields': [
                'title', 'status', 'stage', 'submitter', 'diet_session',
                'submitted_date', 'final_vote_date', 'implementation_date',
                'bill_outline', 'background_context', 'expected_effects',
                'committee_assignments', 'vote_results', 'promulgated_date',
                'data_quality_score'
            ],
            'ignore_fields': [
                'created_at', 'updated_at', 'last_scraped_at', 'scraping_metadata'
            ]
        }
        ...
        try:
            with self.SessionLocal() as session:
                ...
                for i in range(0, len(bills_to_check), batch_size):
                    ...
                    result.history_records_created += batch_result.history_records_created
    ...
    def _get_last_snapshot(self, session: Session, bill_id: str) -> Optional[BillSnapshot]:
    ...
    def _detect_changes(
        ...
        last_snapshot: Optional[BillSnapshot]
    ) -> List[BillChange]:
    ...
    def _infer_change_reason(self, field_name: str, old_value: Any, new_value: Any) -> Optional[str]:
        ...
        try:
            with self.SessionLocal() as session:
                ...
                old_records = session.execute(
                    ...
                    .where(BillProcessHistory.change_type == HistoryChangeType.DATA_CORRECTION)
                ).scalars().all()
        ...
        try:
            with self.SessionLocal() as session:
                ...
                recent_records = session.execute(
                    ...
                    .where(BillProcessHistory.source_system == "auto_history_recorder")
                ).scalars().all()
