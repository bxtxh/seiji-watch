...
class BillProgressTracker:
    ...
    def __init__(self, update_interval_hours: int = 24, enable_alerts: bool = True):
    ...
    def track_bill_progress(
        self, bill_data: EnhancedBillData
    ...
    def _analyze_stage_transitions(
        self, progress_history: list[BillProcessHistory]
    ) -> list[dict[str, Any]]:
        ...
        if bill_data.amendments:
            ...
            if isinstance(latest_amendment, dict):
                return latest_amendment.get(
                    "description", "修正"
                ), latest_amendment.get("date")

        # Fall back to status
        return (
            bill_data.status,
            bill_data.final_vote_date or bill_data.committee_report_date,
        )
    ...
    async def track_multiple_bills(
        self, bills: list[EnhancedBillData]
    ...
    def get_progress_summary(
        self, tracking_results: list[ProgressTrackingResult]
    ) -> dict[str, Any]:
