...
from ...shared.src.shared.models.bill_process_history import BillProcessHistory, HistoryEventType
from ..migration.data_quality_auditor import DataQualityAuditor, QualityReport
...
class DataQualityDashboard:
    ...
    def _create_migration_status_panel(self) -> DashboardPanel:
        ...
        severity = MetricSeverity.SUCCESS if success_rate > 0.9 else MetricSeverity.WARNING
    ...
    def _create_system_health_panel(self) -> DashboardPanel:
        ...
        severity = MetricSeverity.SUCCESS if db_health > 0.95 else MetricSeverity.WARNING
        ...
        severity = MetricSeverity.SUCCESS if service_availability > 0.99 else MetricSeverity.WARNING
    ...
    def _create_trends_panel(self) -> DashboardPanel:
        ...
        severity = MetricSeverity.SUCCESS if quality_trend > 0 else MetricSeverity.WARNING
    ...
    def _get_quality_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            metrics = {
                ...
                'critical_issues': len([i for i in audit_report.issues if i.severity.value == 'critical'])
            }
    ...
    def _get_processing_metrics(self) -> Dict[str, float]:
        ...
        try:
            with self.SessionLocal() as session:
                ...
                daily_volume = session.execute(
                    select(func.count(Bill.id)).where(Bill.updated_at >= cutoff_time)
                ).scalar() or 0
    ...
    def get_alerts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            overall_score = quality_metrics.get('overall_score', 0)
            if overall_score < self.config['alert_thresholds']['quality_score_critical']:
                alerts.append({
                    'type': 'critical',
            ...
            elif overall_score < self.config['alert_thresholds']['quality_score_warning']:
                alerts.append({
                    'type': 'warning',
