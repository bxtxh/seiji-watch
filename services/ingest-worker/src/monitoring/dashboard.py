...
class DashboardDataProvider:
    ...
    def get_overview_metrics(self) -> dict[str, Any]:
        ...
        total_processed = sum(
            stats.get("total_processed", 0) for stats in processing_stats.values()
        )
        ...
        total_successful = sum(
            stats.get("successful", 0) for stats in processing_stats.values()
        )
    ...
    def _determine_pipeline_status(self, stats: dict[str, Any]) -> str:
        ...
        if last_processed:
            ...
            if last_processed and (
                datetime.utcnow() - last_processed.replace(tzinfo=None)
            ) > timedelta(hours=2):
                return "idle"
        ...
        resources = {
            "cpu": {
                ...
                "status": self._get_resource_status(
                    system_metrics.get("cpu_percent", 0), [70, 85]
                ),
            },
            "memory": {
                "current_percent": system_metrics.get("memory_percent", 0),
                "status": self._get_resource_status(
                    system_metrics.get("memory_percent", 0), [80, 90]
                ),
            },
            "disk": {
                "current_percent": system_metrics.get("disk_usage_percent", 0),
                "status": self._get_resource_status(
                    system_metrics.get("disk_usage_percent", 0), [85, 95]
                ),
            },
            ...
            for metric in metric_list:
                if metric.timestamp >= cutoff_time:
                    if "duration" in metric_name:
                        ...
                        metric_type = metric_name.replace("system_", "").replace(
                            "_percent", ""
                        )
