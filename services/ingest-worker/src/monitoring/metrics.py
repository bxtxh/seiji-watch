...
class IngestWorkerMetrics:
    ...
    def record_processing_operation(
        ...
        tags = {"operation": operation_type, "success": str(success).lower()}
        ...
        self.record_metric(
            f"{operation_type}_operations_total", 1, MetricType.COUNTER, tags
        )
        self.record_metric(
        ...
        stats.avg_processing_time = sum(self.processing_times[operation_type]) / len(
            self.processing_times[operation_type]
        )
        ...
        if quality_score is not None:
            ...
            self.record_metric(
                f"{operation_type}_quality_score", quality_score, MetricType.GAUGE, tags
            )
    ...
    def record_pdf_processing(
        ...
        self.record_processing_operation(
            "pdf_processing", success, processing_time, confidence_score, details
        )

        # PDF-specific metrics
        self.record_metric(
            "pdf_pages_processed_total", pages_processed, MetricType.COUNTER, details
        )
        self.record_metric(
    ...
    def record_stt_processing(
        ...
        self.record_processing_operation(
            "stt_processing", success, processing_time, confidence_score, details
        )
        ...
        self.record_metric(
            "stt_words_generated_total", word_count, MetricType.COUNTER, details
        )

        if success and audio_duration > 0:
            wpm = word_count / (audio_duration / 60)  # Words per minute
            self.record_metric(
                "stt_words_per_minute", wpm, MetricType.GAUGE, details, "wpm"
            )
    ...
    def record_vector_processing(
        ...
        details = {"chunks": str(text_chunks), "dimension": str(embedding_dimension)}

        self.record_processing_operation(
            "vector_processing", success, processing_time, similarity_score, details
        )

        # Vector-specific metrics
        self.record_metric(
            "vector_chunks_processed_total", text_chunks, MetricType.COUNTER, details
        )
        self.record_metric(
            "vector_embedding_dimension", embedding_dimension, MetricType.GAUGE, details
        )
    ...
    def record_data_pipeline_operation(
        ...
        self.record_processing_operation(
            "data_pipeline", success, processing_time, data_completeness, details
        )
    ...
    def record_system_metrics(self):
        ...
        try:
            ...
            self.record_metric(
                "system_cpu_percent", cpu_percent, MetricType.GAUGE, tags, "percent"
            )
            self.record_metric(
            ...
            self.record_metric(
                "system_disk_percent", disk_percent, MetricType.GAUGE, tags, "percent"
            )
    ...
    def get_prometheus_metrics(self) -> str:
        ...
        lines.append('ingest_worker_info{version="1.0.0"} 1')
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            self.metrics.record_processing_operation(
                self.operation_type, self.success, duration, **self.kwargs
            )
...
def record_pdf_processing(
    pdf_url: str, success: bool, processing_time: float, **kwargs
):
    """Record PDF processing metrics."""
...
def record_stt_processing(
    audio_duration: float, success: bool, processing_time: float, **kwargs
):
    """Record STT processing metrics."""
...
def record_vector_processing(
    text_chunks: int, success: bool, processing_time: float, **kwargs
):
