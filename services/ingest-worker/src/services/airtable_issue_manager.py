...
class AirtableIssueRecord:
    ...
    parent_id: Optional[str] = None  # Self-relation (null for lv1)
    ...
    valid_to: Optional[date] = None  # Nullable, for versioning
    
    # Metadata
    source_bill_id: Optional[str] = None  # Link to originating bill
    created_at: datetime = field(default_factory=datetime.now)
    updated_at: datetime = field(default_factory=datetime.now)
    reviewer_notes: Optional[str] = None  # Optional review comments
...
class AirtableIssueManager:
    ...
    async def create_unclassified_issue_pair(self, bill_id: str) -> Tuple[str, str]:
        ...
        return await self.create_issue_pair(unclassified_issue, bill_id, quality_score=0.1)
    
    async def get_issue_record(self, record_id: str) -> Optional[AirtableIssueRecord]:
    ...
    async def batch_create_issue_pairs(self, dual_issues: List[Tuple[DualLevelIssue, str]], 
                                     quality_scores: Optional[List[float]] = None) -> List[Tuple[str, str]]:
        ...
        for i in range(0, len(dual_issues), self.batch_size // 2):  # Divide by 2 since we create 2 records per issue
            ...
            for j, (dual_issue, bill_id) in enumerate(batch):
                try:
                    ...
                    lv1_id, lv2_id = await self.create_issue_pair(dual_issue, bill_id, quality_score)
