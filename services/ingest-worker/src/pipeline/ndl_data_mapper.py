...
from shared.src.shared.models.member import Member
from shared.src.shared.models.party import Party
...
class SpeakerNormalizer:
    ...
    @classmethod
    def normalize_party_name(cls, party: Optional[str]) -> Optional[str]:
        """政党名の表記揺れを統一"""
        if not party:
            return None
    ...
    @classmethod
    def extract_speaker_info(cls, speaker_name: str, speaker_group: Optional[str]) -> Dict[str, Optional[str]]:
        """発言者情報を抽出・正規化"""
        return {
...
class NDLDataMapper:
    ...
    def _determine_speaker_type(self, speaker_info: Dict[str, Optional[str]]) -> str:
    ...
    async with NDLAPIClient() as client:
        ...
        if meetings:
            ...
            if meeting_result.success:
                print(f"✅ Meeting mapped successfully")
