...
from ..scraper.enhanced_hr_scraper import EnhancedVotingSession, EnhancedHRProcessor
...
class HRDataIntegrator:
    ...
    async def integrate_hr_voting_data(
        self,
        enhanced_sessions: List[EnhancedVotingSession],
        dry_run: bool = False
    ) -> IntegrationResult:
        """
            ...
            with get_db_session() as db_session:
                for session in enhanced_sessions:
                    try:
                        ...
                        result.vote_records_created += session_result['vote_records_created']
                        result.conflicts_detected += session_result['conflicts_detected']
        ...
        try:
            ...
            if bill:
                ...
                existing_bill = db_session.query(Bill).filter(
                    Bill.bill_number == session.base_session.bill_number
                ).first()
            ...
            member_results = await self._process_member_data(session, db_session, dry_run)
            ...
            for vote_record in session.base_session.vote_records:
                try:
                    ...
                    member = db_session.query(Member).filter(
                        Member.name == vote_record.member_name
                    ).first()
                    ...
                    existing_record = db_session.query(VoteRecord).filter(
                        and_(
