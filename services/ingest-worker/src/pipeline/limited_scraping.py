...
class LimitedScrapeCoordinator:
    ...
    async def execute_limited_scraping(
        self, target: ScrapeTarget | None = None, dry_run: bool = False
    ) -> PipelineResult:
        ...
        try:
            ...
            if target.enable_embeddings and self.vector_client:
                ...
                embedding_result = await self._generate_embeddings_limited(
                    target, dry_run
                )
    ...
    async def _collect_bills_limited(
        self, target: ScrapeTarget, dry_run: bool
    ) -> dict[str, Any]:
        ...
        try:
            ...
            bills_data = await self.diet_scraper.fetch_current_bills_async(
                force_refresh=True
            )
    ...
    async def _process_speeches_limited(
        ...
        try:
            ...
            return {"count": 0, "transcriptions": 0, "errors": []}
    ...
    async def _generate_embeddings_limited(
        ...
        try:
            ...
            return {"count": embeddings_generated, "errors": []}

        except Exception as e:
            self.logger.error(f"Embedding generation failed: {e}")
            return {"count": 0, "errors": [f"Embedding generation error: {str(e)}"]}
