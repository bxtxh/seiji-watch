...
from .diet_scraper import DietScraper, BillData
...
class EnhancedBillData(BillData):
    ...
    bill_outline: Optional[str] = None           # 議案要旨相当の長文情報
    background_context: Optional[str] = None     # 提出背景・経緯
    expected_effects: Optional[str] = None       # 期待される効果
    key_provisions: Optional[List[str]] = field(default_factory=list)  # 主要条項リスト
    related_laws: Optional[List[str]] = field(default_factory=list)    # 関連法律リスト
    implementation_date: Optional[str] = None    # 施行予定日
    
    # Enhanced submission information
    submitting_members: Optional[List[str]] = field(default_factory=list)  # 提出議員一覧
    submitting_party: Optional[str] = None       # 提出会派
    sponsoring_ministry: Optional[str] = None    # 主管省庁
    
    # Process tracking
    committee_assignments: Optional[Dict[str, Any]] = field(default_factory=dict)  # 委員会付託情報
    voting_results: Optional[Dict[str, Any]] = field(default_factory=dict)         # 採決結果
    amendments: Optional[List[Dict[str, Any]]] = field(default_factory=list)       # 修正内容
    inter_house_status: Optional[str] = None     # 両院間の状況
    
    # Source metadata
    source_house: str = "参議院"                 # データ取得元議院
    source_url: Optional[str] = None             # 元データURL
    data_quality_score: Optional[float] = None   # データ品質スコア
