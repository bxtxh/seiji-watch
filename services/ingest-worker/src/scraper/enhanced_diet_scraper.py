...
class EnhancedDietScraper(DietScraper):
    ...
    def _extract_basic_info(
        self, soup: BeautifulSoup, bill_data: EnhancedBillData
    ) -> None:
        ...
        status_text = self._find_text_by_patterns(
            soup, ["状況", "現在の状況", "ステータス"]
        )
        if status_text:
    ...
    def _extract_amendments(
        ...
        if amendment_text:
            amendments.append(
                {"type": "amendment", "description": amendment_text, "date": None}
            )
