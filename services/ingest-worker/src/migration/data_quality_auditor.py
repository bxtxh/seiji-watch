...
class DataQualityAuditor:
    ...
    def __init__(self, database_url: str):
        ...
        self.required_fields = [
            'bill_id', 'title', 'status', 'submitter', 'diet_session', 
            'house_of_origin', 'submitted_date'
        ]
        
        # Enhanced fields to audit
        self.enhanced_fields = [
            'bill_outline', 'background_context', 'expected_effects',
            'key_provisions', 'source_house', 'data_quality_score'
        ]
            ...
            if isinstance(value, str) and value.strip():
                if not self._is_japanese_text_quality_good(value):
                    issues.append(QualityIssue(
                        ...
                        current_value=value[:100] + "..." if len(value) > 100 else value,
    ...
    def _check_data_freshness(self, bill: Bill) -> List[QualityIssue]:
        ...
        if bill.updated_at:
            ...
            if days_old > self.quality_thresholds['timeliness_days']:
                severity = QualityIssueSeverity.HIGH if days_old > 90 else QualityIssueSeverity.MEDIUM
