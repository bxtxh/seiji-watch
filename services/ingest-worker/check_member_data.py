        ...
        async with session.get(
            ...
            if response.status == 200:
                ...
                if total_dummy > total_real:
                    print("\n❌ 問題発見!")
                    print("  議員テーブルの大部分がダミーデータです")
                    print("  実際の国会議員データに置き換える必要があります")
                    print("\n💡 推奨対応:")
                    print("  1. 参議院・衆議院の公式議員リストをスクレイピング")
                    print("  2. 実際の議員名、選挙区、所属政党データを収集")
                    print("  3. ダミーデータを実データで置換")
                else:
                    print("\n✅ データ品質良好")
                    print("  実データが多数を占めています")
