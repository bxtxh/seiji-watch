...
class MemberDataVerifier:
    ...
    def analyze_member_data(self, members: list, parties: dict) -> dict:
        ...
        for member in members:
            ...
            if party_links:
                party_id = (
                    party_links[0] if isinstance(party_links, list) else party_links
                )
    ...
    def print_verification_report(self, stats: dict):
        ...
        sorted_parties = sorted(
            stats["party_distribution"].items(), key=lambda x: x[1], reverse=True
        )
        ...
        sorted_constituencies = sorted(
            stats["constituency_distribution"].items(), key=lambda x: x[1], reverse=True
        )[:10]
        ...
        completeness_score = (
            sum(stats["data_completeness"].values())
            / (stats["total_members"] * total_fields)
            * 100
        )
