...
class IncompleteKanaAnalyzer:
    ...
    def __init__(self):
        ...
        self.common_surnames = [
            'たなか', 'さとう', 'すずき', 'たかはし', 'いとう', 'わたなべ', 'やまもと', 'なかむら',
            'こばやし', 'かとう', 'よしだ', 'やまだ', 'ささき', 'やまぐち', 'まつもと', 'いのうえ',
            'きむら', 'はやし', 'さいとう', 'しみず', 'やまざき', 'もり', 'あべ', 'いけだ',
            'はしもと', 'やました', 'いしかわ', 'なかじま', 'まえだ', 'ふじた', 'ごとう', 'おかだ',
            'はせがわ', 'むらかみ', 'こんどう', 'いしだ', 'にしむら', 'まつだ', 'はらだ', 'わだ',
            'ふくだ', 'おおた', 'うえだ', 'もりた', 'たむら', 'たけだ', 'むらた', 'にった',
            'おがわ', 'なかがわ', 'あおき', 'きのした', 'おおしま', 'しまだ', 'ふじわら', 'みうら'
        ]
        ...
        if kana_len < expected_min_kana:
            ...
            if any(surname in name_kana for surname in self.common_surnames):
                if name_len > 2:  # Multi-character name but short kana
                    return "surname_only", f"Likely surname-only: {name} → {name_kana} (expected ≥{expected_min_kana:.0f} kana)"
            
            return "too_short", f"Too short: {kana_len} kana for {name_len} kanji (expected ≥{expected_min_kana:.0f})"
    ...
    def print_incomplete_analysis_report(self):
        ...
        print(f"\n{'='*80}")
        print(f"🔍 INCOMPLETE NAME_KANA ANALYSIS REPORT")
        print(f"{'='*80}")
        ...
        if results['total_analyzed'] > 0:
            ...
            if completeness_rate >= 95:
                ...
                print(f"   🎯 VERY GOOD completeness")
            elif completeness_rate >= 80:
                print(f"   👍 GOOD completeness")
            else:
                print(f"   ⚠️ NEEDS IMPROVEMENT - many incomplete readings")
...
async def main():
    ...
    if total_incomplete > 0:
        ...
        print(f"🎉 All Name_Kana readings appear to be complete!")
