...
class RealMemberDataCollector:
    ...
    async def scrape_sangiin_members(self, session: aiohttp.ClientSession) -> List[RealMemberData]:
        ...
        # 参議院議員名簿ページ
        sangiin_url = "https://www.sangiin.go.jp/japanese/joho1/kousei/giin/217/giin.htm"
        
        members = []
        
        try:
            async with session.get(sangiin_url, headers=self.scraping_headers) as response:
    ...
    async def scrape_shugiin_members(self, session: aiohttp.ClientSession) -> List[RealMemberData]:
        ...
        try:
            async with session.get(shugiin_url, headers=self.scraping_headers) as response:
                if response.status == 200:
                    html = await response.text()
                    soup = BeautifulSoup(html, 'html.parser')
        ...
        try:
            ...
            for record in records:
                ...
                is_dummy = (
                    ...
                    name in ["山田太郎", "田中花子", "佐藤次郎", "鈴木三郎", "高橋美咲"]
                )
    ...
    async def collect_and_replace_member_data(self) -> Dict:
        ...
        print(f"🎯 目標: ダミーデータを実際の国会議員データで置換")
        print()
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                print(f"\n💾 Step 4: 実議員データ投入...")
                success_count = await self.insert_real_members(session, members, party_id_map)
                ...
                if result["success"]:
                    ...
                    print(f"✅ ダミーデータを実議員データで置換完了")
                    print(f"✅ 議員データベースの信頼性向上")
                    print(f"🔄 Ready for production API usage")
                else:
                    print(f"\n⚠️  部分的成功: さらなるデータ収集推奨")
