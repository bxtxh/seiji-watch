...
def get_all_bills_with_notes(pat, base_id):
    ...
    params = {
        "maxRecords": 100,
        "fields": ["Name", "Notes", "Bill_ID", "Data_Source", "Collection_Date"],
    }
...
def suggest_migration_strategy(pattern_types, extractable_fields):
    ...
    for pattern, items in pattern_types.items():
        ...
        if pattern == "structured_emoji":
            print(
                f"  📋 {pattern}: {count}件 ({percentage:.1f}%) → 構造化データ解析で完全移行可能"
            )
        elif pattern == "detailed_newline":
            print(
                f"  📝 {pattern}: {count}件 ({percentage:.1f}%) → 正規表現で部分移行可能"
            )
        elif pattern == "pipe_separated":
            print(
                f"  ⚡ {pattern}: {count}件 ({percentage:.1f}%) → パイプ分割で簡単移行"
            )
        elif pattern == "multiline_detailed":
            print(
                f"  📄 {pattern}: {count}件 ({percentage:.1f}%) → 行単位解析で移行可能"
            )
        else:
