...
async def execute_simplified_production_scraping(diet_scraper, voting_scraper, target):
    ...
    try:
        ...
        result = {
            ...
            'errors': [],
            'performance_metrics': {
                'bills_per_second': len(bills_data) / total_time if total_time > 0 else 0,
                'voting_sessions_per_second': len(voting_data) / total_time if total_time > 0 else 0,
...
async def execute_production_scraping():
    ...
    try:
        ...
        result = await execute_simplified_production_scraping(diet_scraper, voting_scraper, target)
        ...
        result_data = {
            ...
            'data_quality_summary': {
                'collection_completeness': result['bills_collected'] / target['max_bills'] if target['max_bills'] > 0 else 0,
                'voting_completeness': result['voting_sessions_collected'] / target['max_voting_sessions'] if target['max_voting_sessions'] > 0 else 0,
                'speech_completeness': result['speeches_processed'] / target['max_speeches'] if target['max_speeches'] > 0 else 0,
                'ai_processing_success': result['embeddings_generated'] > 0 and result['transcriptions_completed'] >= 0,
                'overall_success_rate': (result['bills_collected'] + result['voting_sessions_collected'] + result['speeches_processed']) / (target['max_bills'] + target['max_voting_sessions'] + target['max_speeches']) if (target['max_bills'] + target['max_voting_sessions'] + target['max_speeches']) > 0 else 0
            },
            'production_readiness': {
                ...
                'error_rate': len(result['errors']) / max(1, result['bills_collected'] + result['voting_sessions_collected'] + result['speeches_processed']),
