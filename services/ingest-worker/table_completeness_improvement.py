...
class TableCompletenessImprover:
        ...
        self.table_strategies = {
            "Bills (法案)": {
                "priority_fields": [
                    ...
                    "House", "Submitter", "Category"
                ],
                "enhancement_fields": [
                    "Priority", "Stage", "Bill_Type", "Process_Method"
                ],
            ...
            "Members (議員)": {
                "priority_fields": [
                    "Name", "House", "Constituency", "Party", "Is_Active"
                ],
                "enhancement_fields": [
                    "Name_Kana", "First_Elected", "Terms_Served"
                ],
            ...
            "Speeches (発言)": {
                "priority_fields": [
                    "Speaker_Name", "Speech_Content", "Speech_Date", 
                    "Meeting_Name", "House"
                ],
                "enhancement_fields": [
                    "Category", "Duration_Minutes", "Topics", "Sentiment", "AI_Summary"
                ],
            ...
            for field, field_analysis in analysis["field_analysis"].items():
                if field_analysis["completeness_rate"] < 0.80:  # Less than 80% complete
                    ...
                    opportunities.append({
                        ...
                        "priority": "high" if field in strategy.get("priority_fields", []) else "medium"
                    })
    ...
    async def implement_bills_improvements(self, session: aiohttp.ClientSession, records: List[Dict]) -> Dict:
        ...
        for record in records:
            ...
            if updates:
                success = await self.safe_update_record(session, "Bills (法案)", record_id, updates)
                if not success:
    ...
    async def implement_members_improvements(self, session: aiohttp.ClientSession, records: List[Dict]) -> Dict:
        ...
        for record in records:
            ...
            # Apply updates if any
            if updates:
                success = await self.safe_update_record(session, "Members (議員)", record_id, updates)
                if not success:
    ...
    async def implement_speeches_improvements(self, session: aiohttp.ClientSession, records: List[Dict]) -> Dict:
        ...
        for record in records:
            ...
            # Apply updates if any
            if updates:
                success = await self.safe_update_record(session, "Speeches (発言)", record_id, updates)
                if not success:
    ...
    async def implement_parties_improvements(self, session: aiohttp.ClientSession, records: List[Dict]) -> Dict:
        ...
        for record in records:
            ...
            # Apply updates if any
            if updates:
                success = await self.safe_update_record(session, "Parties (政党)", record_id, updates)
                if not success:
        ...
        async with aiohttp.ClientSession() as session:
            for table_name in self.table_strategies.keys():
                ...
                if table_name == "Bills (法案)":
                    improvement_results = await self.implement_bills_improvements(session, records)
                elif table_name == "Members (議員)":
                    improvement_results = await self.implement_members_improvements(session, records)
                elif table_name == "Speeches (発言)":
                    improvement_results = await self.implement_speeches_improvements(session, records)
                elif table_name == "Parties (政党)":
                    improvement_results = await self.implement_parties_improvements(session, records)
    ...
    async def generate_improvement_report(self, results: Dict):
        ...
        # Print summary
        print(f"\n{'='*80}")
        print(f"📋 TABLE COMPLETENESS IMPROVEMENT SUMMARY")
        print(f"{'='*80}")
...
async def main():
    ...
    results = await improver.run_table_improvements()
