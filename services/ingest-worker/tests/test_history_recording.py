...
from ..src.processor.bill_history_recorder import (
    BillHistoryRecorder, BillChange, BillSnapshot, 
    ChangeDetectionMode, ChangeSignificance,
    HistoryRecordingResult
)
from ..src.scheduler.history_recording_scheduler import (
    HistoryRecordingScheduler, ScheduleConfig, ScheduleFrequency
)
from ..src.services.history_service import (
    HistoryService, HistoryQuery, HistoryAnalytics
)
from ...shared.src.shared.models.bill import Bill
from ...shared.src.shared.models.bill_process_history import (
    BillProcessHistory, HistoryEventType, HistoryChangeType
)
...
class TestBillHistoryRecorder:
    ...
    def test_is_significant_change(self, history_recorder):
        ...
        # Significant change
        assert history_recorder._is_significant_change("status", "提出", "審議中") == True
        
        # Non-significant change
        assert history_recorder._is_significant_change("title", "Test Bill", "Test Bill") == False
        
        # None to value
        assert history_recorder._is_significant_change("title", None, "Test Bill") == True
        
        # Value to None
        assert history_recorder._is_significant_change("title", "Test Bill", None) == True
        
        # Similar strings (should be non-significant)
        assert history_recorder._is_significant_change("title", "Test Bill", "Test Bill Updated") == False
    ...
    def test_detect_and_record_changes_success(self, mock_session_maker, history_recorder):
        ...
        mock_bill.title = "Test Bill"
        mock_bill.status = "審議中"
        mock_bill.updated_at = datetime.now(timezone.utc)
...
class TestHistoryRecordingScheduler:
    ...
    def test_scheduler_initialization(self, scheduler):
        ...
        assert scheduler.status.is_running == False
...
class TestHistoryService:
    ...
    def test_record_manual_change(self, history_service):
        ...
        with patch.object(history_service.history_recorder, 'record_manual_change') as mock_record:
            ...
            mock_history_record.recorded_at = datetime.now(timezone.utc)
    ...
    def test_get_scheduler_status_initialized(self, history_service):
        ...
        assert status['status']['is_running'] == True
