...
from ..src.monitoring.data_quality_dashboard import (
    DataQualityDashboard, DashboardMetric, DashboardPanel, DashboardLayout,
    DashboardMetricType, MetricSeverity
)
from ..src.monitoring.monitoring_service import (
    MonitoringService, AlertRule, Alert, HealthCheck, MonitoringStats,
    AlertType, AlertSeverity, NotificationChannel
)
from ..src.monitoring.monitoring_manager import (
    MonitoringManager, MonitoringConfiguration
)
...
class TestDataQualityDashboard:
    ...
    def test_create_main_dashboard(self, dashboard):
        ...
        dashboard._get_quality_metrics = Mock(return_value={
            'overall_score': 0.85,
        ...
        # Mock other metrics
        dashboard._get_processing_metrics = Mock(return_value={
            'daily_volume': 150,
            'error_rate': 0.02,
            'avg_processing_time': 2.5
        })
        
        dashboard._get_migration_metrics = Mock(return_value={
            'success_rate': 0.95,
            'total_migrations': 10,
            'recent_migrations': 3
        })
        
        dashboard._get_system_health_metrics = Mock(return_value={
            'database_health': 1.0,
            'service_availability': 0.995
        })
        
        dashboard._get_trend_metrics = Mock(return_value={
            'quality_trend': 0.05,
            'volume_trend': 0.1
        })
...
class TestMonitoringService:
    ...
    def test_health_check_creation(self, monitoring_service):
        ...
        assert check_dict['enabled'] == True
    ...
    def test_alert_triggering(self, monitoring_service):
        ...
        # Mock metrics
        monitoring_service._get_current_metrics = Mock(return_value={
    ...
    def test_health_check_execution(self, monitoring_service):
        ...
        assert monitoring_service.health_check_results['test_check']['success'] == True
...
class TestMonitoringManager:
    ...
    def test_manager_initialization(self, monitoring_manager):
        ...
        assert monitoring_manager.is_running == False
    ...
    def test_start_stop_manager(self, monitoring_manager):
        ...
        assert monitoring_manager.is_running == True
    ...
    def test_cleanup_old_data(self, monitoring_manager):
        ...
        assert monitoring_manager.monitoring_service.alert_history[0].alert_id == "recent_alert"
...
class TestIntegrationScenarios:
    ...
    def test_end_to_end_monitoring_flow(self):
        ...
        with patch('sqlalchemy.create_engine') as mock_engine_create:
            ...
            manager.dashboard.get_dashboard_data = Mock(return_value={
                'dashboard_id': 'test',
                'panels': [
                    {
                        'panel_id': 'quality_overview',
    ...
    def test_alert_workflow(self):
        ...
        with patch('sqlalchemy.create_engine') as mock_engine_create:
            ...
            service._get_current_metrics = Mock(return_value={
                'quality_score': 0.4,  # Below critical threshold
                'completeness_rate': 0.5,  # Below critical threshold
                'error_rate': 0.15  # Above critical threshold
            })
    ...
    def test_health_check_workflow(self):
        ...
        with patch('sqlalchemy.create_engine') as mock_engine_create:
            ...
            # Check results
            assert service.health_check_results['database_connectivity']['success'] == True
            assert service.health_check_results['data_quality_service']['success'] == True
            assert service.health_check_results['migration_service']['success'] == False
            assert service.health_check_results['disk_space']['success'] == True
