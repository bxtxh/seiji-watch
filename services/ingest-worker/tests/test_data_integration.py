...
from ..src.processor.bill_data_merger import (
    BillDataMerger, MergeResult, MergeConflict, 
    ConflictResolutionStrategy, MergeStrategy
)
from ..src.processor.bill_data_validator import (
    BillDataValidator, ValidationResult, ValidationIssue, 
    ValidationSeverity, ValidationCategory
)
from ..src.processor.bill_progress_tracker import (
    BillProgressTracker, ProgressUpdateResult, StageTransition,
    ProgressAlert, AlertType
)
from ..src.processor.data_integration_manager import (
    DataIntegrationManager, IntegrationResult, ProcessingStage
)
...
class TestIntegrationScenarios:
    ...
    def test_error_handling_and_recovery(self):
        ...
        with patch('services.ingest-worker.src.processor.data_integration_manager.BillDataMerger') as mock_merger_class:
            ...
            result1 = manager.process_bills([{'bill_id': 'TEST-001', 'title': 'テスト法案'}])
            assert result1.success is False
            
            # Second attempt should succeed
            result2 = manager.process_bills([{'bill_id': 'TEST-001', 'title': 'テスト法案'}])
    ...
    def test_data_quality_improvement(self):
        ...
        with patch('services.ingest-worker.src.processor.data_integration_manager.BillDataValidator') as mock_validator_class:
            mock_validator = Mock()
            mock_validator_class.return_value = mock_validator
