...
async def test_batch_processor_api():
    ...
    try:
        from batch_queue.batch_processor import (
            BatchProcessor, BatchConfig, TaskType, TaskPriority
        )
...
async def test_resilient_scraper_api():
    ...
    try:
        from scraper.resilience import (
            ResilientScraper, RateLimitConfig, CacheConfig
        )
...
async def test_pdf_processor_basic():
    ...
    try:
        from scraper.pdf_processor import PDFProcessor, MemberNameMatcher
        ...
        print(f"✓ PDF processor statistics available")
