...
class CompleteSanguinMemberCollector:
    ...
    def __init__(self):
        ...
        self.member_list_url = (
            f"{self.sangiin_base_url}/japanese/joho1/kousei/giin/current/giin.htm"
        )
        ...
        try:
                        ...
                        try:
                            ...
                            if name_link:
                                ...
                                if profile_url and not profile_url.startswith("http"):
                                    profile_url = (
                                        f"{self.sangiin_base_url}{profile_url}"
                                    )
                            else:
                            ...
                            party_name = (
                                party_cell.get_text(strip=True) if party_cell else None
                            )
                            ...
                            constituency, election_type = (
                                self.extract_constituency_info(constituency_text)
                            )
            ...
            for url in additional_urls:
                try:
                    ...
                    if html_content:
                        soup = BeautifulSoup(html_content, "html.parser")
                        additional_members = await self.parse_additional_member_page(
                            soup
                        )
            ...
            for row in rows:
                ...
                if len(cells) >= 2:
                    try:
                        ...
                        # 議員名の妥当性チェック
                        if len(name_text) > 1 and not any(
                            word in name_text.lower()
                            for word in ["氏名", "議員名", "name", "政党"]
                        ):
                            member_data = SanguinMemberData(
                                name=name_text, house="参議院", is_active=True
                            )
                            members.append(member_data)
                    except Exception:
                        continue
    ...
    async def get_existing_parties(
        ...
        try:
            await self.rate_limit_delay()
            async with session.get(
                f"{self.base_url}/Parties (政党)", headers=self.headers
            ) as response:
    ...
    async def create_party_if_not_exists(
        self,
        session: aiohttp.ClientSession,
        party_name: str,
        existing_parties: dict[str, str],
    ) -> str | None:
        ...
        try:
            ...
            async with session.post(
                f"{self.base_url}/Parties (政党)", headers=self.headers, json=party_data
            ) as response:
                if response.status == 200:
    ...
    async def insert_members_to_airtable(
        self, session: aiohttp.ClientSession, members: list[SanguinMemberData]
    ) -> bool:
            ...
            for i in range(0, len(members), batch_size):
                ...
                for member in batch:
                    ...
                    if member.party_name and member.party_name != "無所属":
                        party_id = await self.create_party_if_not_exists(
                            session, member.party_name, existing_parties
                        )
                    ...
                    if member.previous_occupations:
                        record_fields["Previous_Occupations"] = (
                            member.previous_occupations
                        )
                    if member.education:
    ...
    async def save_results_to_file(
        self, members: list[SanguinMemberData], filename: str = None
    ):
