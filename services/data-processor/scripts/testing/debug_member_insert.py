...
async def debug_member_insert():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        minimal_data = {"fields": {"Name": "テスト議員"}}

        async with session.post(
            f"{base_url}/Members (議員)", headers=headers, json=minimal_data
        ) as response:
            ...
            if response.status == 200:
                ...
                async with session.delete(
                    f"{base_url}/Members (議員)/{record_id}", headers=headers
                ) as del_response:
        ...
        enhanced_data = {
            "fields": {"Name": "テスト議員２", "House": "参議院", "Is_Active": True}
        }

        async with session.post(
            f"{base_url}/Members (議員)", headers=headers, json=enhanced_data
        ) as response:
            print(f"  Status: {response.status}")
            if response.status == 200:
                result = await response.json()
