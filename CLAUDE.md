...
Typical Claude prompts 1. Use playwright mcp to run all e2e tests 2. Use playwright mcp to open http://localhost:3000 in headed mode 3. Use playwright mcp to reproduce failing test tests/e2e/login.spec.ts:15
...
• Specs live in tests/e2e/\*_/_.spec.ts
• Screenshots/videos are saved to test-results/ by default

Debug workflow 1. Trigger a headed run: PWDEBUG=1 npx playwright test <file> or ask Claude with prompt #2 above. 2. Inspect the browser or use Playwright Inspector to step through. 3. Fix the issue, then rerun tests.
...
Current Limits: 30,000 TPM (Tokens Per Minute) - approximately 10-15 complex queries per minute 1. Batch Questions Efficiently:
...
- "FastAPI CORS preflight returning 400. I have CORSMiddleware configured but
  ...
  3.  Fallback Strategies:
      • Use Gemini Flash (mcp**gemini**gemini_chat_flash) for simpler questions
