...
AIRTABLE_PAT = (
    "patzu6qz1qNDVGZqL.7feb60b33535807523001c8ee9d368040fe757652b387f9e333f688583747144"
)
            ...
            for table_name in table_names:
                ...
                async with session.get(url, headers=headers) as resp:
                    if resp.status == 200:
                        ...
                        print(
                            f"   ✅ Found {len(records)} bills in table '{table_name}'"
                        )
                        for record in records:
                            fields = record.get("fields", {})
                            name = fields.get(
                                "Name",
                                fields.get("名前", fields.get("title", "No name")),
                            )
                            print(f"      - {name[:50]}...")
                        success = True
                        break
