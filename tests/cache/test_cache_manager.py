...
from shared.cache.cache_warmer import (
    CacheWarmer,
    WarmingTask,
    WarmingPriority
)
...
class TestCacheManager:
    ...
    async def test_get_cache_hit(self, cache_manager):
        ...
        cache_manager.redis_client.get.return_value = cache_manager._serialize(test_data)
    ...
    async def test_invalidate_pattern(self, cache_manager):
        ...
        cache_manager.redis_client.scan.return_value = (
            0, [b"cache:bills:1", b"cache:bills:2"]
        )
    ...
    async def test_analyze_patterns(self, cache_manager):
        ...
        cache_manager.access_patterns["cache:bills:2"] = [
...
class TestCacheWarmer:
    ...
    async def test_warm_task(self, cache_warmer):
        ...
        cache_warmer.supabase_client.get_parties = AsyncMock(
    ...
    async def test_warm_by_priority(self, cache_warmer):
        ...
        cache_warmer.supabase_client.get_members = AsyncMock(
            return_value=[{"id": "1"}]
        )
        
        # Warm critical priority tasks
        count = await cache_warmer.warm_by_priority(
...
class TestCacheInvalidator:
    ...
    async def test_invalidate_on_change(self, cache_invalidator):
        ...
        # Mock invalidation
        cache_invalidator.cache_manager.invalidate = AsyncMock(
            return_value=5
        )
