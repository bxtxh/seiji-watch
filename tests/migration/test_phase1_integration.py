...
from shared.dal.hybrid_data_access import HybridDataAccessLayer, DataSource
from shared.migration.data_migration_service import DataMigrationService, MigrationStatus
...
class TestHybridDataAccessLayer:
    ...
    @pytest.mark.asyncio
    async def test_read_preference_supabase_first(self, airtable_client, supabase_client):
    ...
    @pytest.mark.asyncio
    async def test_write_operations_airtable_only(self, airtable_client, supabase_client):
        ...
        airtable_client.create_bill.return_value = {
            "id": "recNew",
            "fields": bill_data
        }
...
class TestDataMigrationService:
    ...
    async def test_migration_validation(self, airtable_client, supabase_client):
        ...
        # Validate
        validation_results = await migration_service.validate_migration(["Bills (法案)"])
        ...
        assert validation_results["tables"]["Bills (法案)"]["consistency_score"] == 100.0
...
class TestSyncService:
    ...
    async def test_incremental_sync(self, airtable_client, supabase_client):
        ...
        # Set last sync time
        sync_service.last_sync_times["Bills (法案)"] = datetime.utcnow() - timedelta(hours=1)
    ...
    async def test_full_sync(self, airtable_client, supabase_client):
        ...
        supabase_client.select.return_value = [
            {"id": "uuid-1", "airtable_id": "recBill1", "updated_at": datetime.utcnow().isoformat()}
        ]
    ...
    async def test_conflict_resolution(self, airtable_client, supabase_client):
        ...
        config = SyncConfig(
            conflict_resolution="airtable_priority"
        )
