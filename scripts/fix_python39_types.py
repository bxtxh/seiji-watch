...
def fix_type_annotations(file_path):
    ...
    content = re.sub(r'\blist\[([^\]]+)\]\s*\|\s*None\b', r'Optional[List[\1]]', content)
    
    # Fix dict types (dict[str, Any] | None -> Optional[Dict[str, Any]])
    content = re.sub(r'\bdict\[([^\]]+)\]\s*\|\s*None\b', r'Optional[Dict[\1]]', content)
    ...
    if 'Optional[' in content or 'List[' in content or 'Dict[' in content:
        ...
        if import_match:
            ...
            content = re.sub(r'^from typing import .*$', new_import_line, content, flags=re.MULTILINE)
        else:
