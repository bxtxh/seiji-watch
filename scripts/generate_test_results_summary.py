...
class TestResultsAnalyzer:
    ...
    def load_test_results(self) -> dict[str, Any]:
        ...
        # Load issue reports
        issue_files = list(self.results_dir.glob('issues/*.json')) + list(self.results_dir.glob('issues/*.md'))
        ...
        self.logger.info(f"Loaded results: {len(results['phase_results'])} phases, "
    ...
    def _load_phase_results(self, phase_dir: Path) -> dict[str, Any]:
        ...
        if phase_results['test_cases']:
            completed = sum(1 for tc in phase_results['test_cases'] if tc.get('status') == 'PASS')
    ...
    def generate_summary_report(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        summary = {
            ...
            'phase_performance': self._analyze_phase_performance(results['phase_results']),
    ...
    def _analyze_test_execution(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        for phase_name, phase_data in results['phase_results'].items():
            ...
            passed_test_cases += sum(1 for tc in test_cases if tc.get('status') == 'PASS')
            failed_test_cases += sum(1 for tc in test_cases if tc.get('status') == 'FAIL')
    ...
    def _evaluate_success_criteria(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        criteria_evaluation = {
            ...
            'accessibility_requirements': self._check_accessibility_requirements(results),
    ...
    def _check_functional_requirements(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        pass_rate = sum(1 for tc in test_cases if tc.get('status') == 'PASS') / len(test_cases) * 100
    ...
    def _check_accessibility_requirements(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            'met': wcag_compliance.get('actual', False) and lighthouse_a11y.get('actual', 0) >= 90,
    ...
    def _check_legal_compliance(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        legal_issues = [
            issue for issue in results['issue_reports'] 
    ...
    def _make_go_no_go_decision(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        go_decision = (
            ...
            functional_met and
            accessibility_met and
            security_met and
            legal_met and
            high_issues < 5  # Allow up to 5 high issues
        )
        ...
        return {
            ...
            'conditions': self._generate_launch_conditions(results) if go_decision else [],
            'blocking_issues': self._identify_blocking_issues(results) if not go_decision else []
        }
...
"""
        ...
        for category, count in summary['issue_analysis']['category_distribution'].items():
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Generate test results summary and analysis')
