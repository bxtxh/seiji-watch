...
class TestAccountDisabler:
    ...
    def disable_user(self, user_email: str) -> dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"Failed to disable user {user_email}: {response.status_code}"
                )
                return {
                    "email": user_email,
                    "status": "failed",
            ...
            return {"email": user_email, "status": "failed", "error": str(e)}
    ...
    def disable_all_test_accounts(self, credentials_file: str) -> dict[str, Any]:
        ...
        results = {
            ...
            "disabled_at": datetime.utcnow().isoformat(),
            "accounts": {},
            "summary": {"total": 0, "disabled": 0, "failed": 0},
        }
...
def main():
    ...
    parser.add_argument(
        "--credentials", required=True, help="Test credentials JSON file"
    )
    parser.add_argument(
        "--output", default="test-accounts-disabled.json", help="Output report file"
    )
