...
class TestAccountManager:
    ...
    def create_test_user(self, user_id: str, user_data: Dict) -> Dict:
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        self.logger.warning(
                            f"Request timeout for {user_id}, retrying in {retry_delay}s... (attempt {attempt + 1}/{max_retries})"
                        )
                    ...
                    if attempt < max_retries - 1:
                        self.logger.warning(
                            f"Connection error for {user_id}, retrying in {retry_delay}s... (attempt {attempt + 1}/{max_retries})"
                        )
                        time.sleep(retry_delay)
                        retry_delay *= 2
                    else:
            ...
            if response.status_code == 201:
                ...
                self.logger.error(
                    f"Failed to create user {user_id}: {response.status_code} - {response.text}"
                )
                return {
                    "user_id": user_id,
    ...
    def create_all_test_users(self) -> Dict:
        ...
        results = {
            ...
            "created_at": datetime.utcnow().isoformat(),
            "users": {},
            "summary": {"total": len(TEST_USERS), "created": 0, "failed": 0},
        }
        ...
        self.logger.info(
            f"Test account creation complete: {results['summary']['created']}/{results['summary']['total']} successful"
        )
        return results
    ...
    def generate_test_credentials_file(self, results: Dict, output_file: str):
        ...
        credentials = {
            "testing_environment": {
                ...
                "environment": self.environment,
                "frontend_url": self.api_base_url.replace(
        ...
        try:
            ...
            if not project_id:
                raise ValueError(
                    "GCP_PROJECT_ID environment variable required for Secret Manager"
                )
            ...
            try:
                version = client.add_secret_version(
                    request={"parent": secret_name, "payload": {"data": secret_payload}}
                )

                self.logger.info(
                    f"Tokens securely stored in Secret Manager: {version.name}"
                )
            ...
            self.logger.error(
                "google-cloud-secret-manager not installed. Tokens NOT saved."
            )
            print(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create test accounts for external user testing"
    )
    parser.add_argument(
        "--environment", required=True, help="Environment (staging-external-test)"
    )
    parser.add_argument(
        "--api-url", help="API base URL (auto-detected if not provided)"
    )
    parser.add_argument(
        "--output",
        default="external-test-credentials.json",
        help="Output credentials file",
    )
    ...
    if not args.api_url:
        ...
        if not project_id:
            print(
                "Error: GCP_PROJECT_ID environment variable required if --api-url not provided"
            )
            sys.exit(1)
    ...
    if (
        expected_project_id
        and actual_project_id
        and expected_project_id != actual_project_id
    ):
        print(
            f"Warning: GCP_PROJECT_ID mismatch - Expected: {expected_project_id}, Actual: {actual_project_id}"
        )
    ...
    try:
        ...
        print(
            f"   Created: {results['summary']['created']}/{results['summary']['total']} accounts"
        )
